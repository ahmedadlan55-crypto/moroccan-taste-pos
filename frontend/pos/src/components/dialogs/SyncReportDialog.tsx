...
export function SyncReportDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} title="تقرير المزامنة" widthClass="max-w-lg">
        ...
        <div className="text-sm font-extrabold text-slate-600">
          {online ? "متصل بالخادم" : (
            <span className="flex items-center gap-1.5 text-amber-800">
              <CloudOff className="h-4 w-4" aria-hidden /> غير متصل
            </span>
          )}
          <p className="mt-0.5 text-[11px] font-bold text-slate-400">
            <span className="num">{fmtInt(queueCount)}</span> عملية بانتظار المزامنة
          </p>
        </div>
      ...
      {pending.length > 0 ? (
        <div className="mb-4">
          <p className="mb-2 text-xs font-extrabold text-slate-500">قائمة الانتظار</p>
              ...
              <li key={op.opId} className="flex items-center justify-between rounded-xl border border-slate-100 px-3 py-2 text-xs">
                <span className="font-extrabold text-slate-600">
                  ...
                  <span className="ms-2 font-bold text-slate-400">
                    طلب <Money value={shortRef(op.orderId)} />
                  </span>
                </span>
      ...
      <p className="mb-2 text-xs font-extrabold text-slate-500">
        آخر مزامنة {lastReport ? <Money value={fmtDateTime(lastReport.at)} className="font-bold text-slate-400" /> : null}
      </p>
      {!lastReport ? (
        <EmptyState icon={<RotateCw className="h-8 w-8" aria-hidden />} title="لا مزامنة بعد" hint="ستظهر النتائج هنا بعد أول دفعة" />
      ) : (
        <ul className="space-y-1">
          {lastReport.results.map((r) => (
            ...
            >
              <span className="flex items-center gap-1.5 font-extrabold text-slate-600">
                ...
                {OP_LABELS[r.type] ?? r.type}
                <span className="font-bold text-slate-400">
                  طلب <Money value={shortRef(r.orderId)} />
                </span>
                {r.replay ? <span className="chip border-slate-200 bg-white text-slate-400">مُعاد</span> : null}
              </span>
              <span className={cn("font-bold", r.ok ? "text-teal-600" : "text-red-600")}>
                {r.ok ? "نجحت" : r.error || r.code || "فشلت"}
              </span>
            </li>
          ))}
        </ul>
