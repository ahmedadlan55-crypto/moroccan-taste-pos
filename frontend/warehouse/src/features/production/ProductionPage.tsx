...
import { formatCurrency, formatNumber, formatDate, formatQty } from "@/lib/formatters";
import { productionStatusToLabel, PRODUCTION_STATUS_OPTIONS, PRODUCTION_PARTIAL_LABEL } from "@/lib/status-labels";
  ...
  return (
    <div>
      <PageHeader
        ...
        action={canCreate ? (<Button variant="primary" onClick={() => navigate("/production/new")}><Plus className="h-4 w-4" /> أمر إنتاج جديد</Button>) : null}
      />
      ...
      <section className="mt-4">
        {isLoading ? (
          ...
          <EmptyState title="لا توجد أوامر إنتاج مطابقة" body={q || status || warehouseId ? "جرّب تعديل عوامل التصفية." : "ابدأ بإنشاء أمر إنتاج من وصفة (BOM)."} action={canCreate ? <Button onClick={() => navigate("/production/new")}><Plus className="h-4 w-4" /> أمر إنتاج جديد</Button> : undefined} />
        ) : (
          <>
            <div className="surface hidden overflow-hidden md:block">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {data.rows.map((r) => (
                    <tr key={r.id} className="cursor-pointer transition hover:bg-slate-50" onClick={() => navigate(`/production/${r.id}`)}>
            ...
            <div className="space-y-3 md:hidden">
              {data.rows.map((r) => (
                <button key={r.id} type="button" onClick={() => navigate(`/production/${r.id}`)} className="surface block w-full p-4 text-right">
