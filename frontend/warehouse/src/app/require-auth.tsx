...
// Gate the whole app behind a valid session. The warehouse app shares the
// legacy app's JWT (localStorage pos_token); there is no separate login here.
// In dev/preview we render the UI even without a token so the foundation is
  ...
  return (
    <div className="grid min-h-screen place-items-center p-6">
      <div className="surface grid max-w-md place-items-center gap-4 p-10 text-center">
        <span className="grid h-14 w-14 place-items-center rounded-2xl bg-teal-500 text-white">
          <Warehouse className="h-6 w-6" />
        </span>
        <h1 className="text-xl font-extrabold text-slate-900">مركز المستودعات</h1>
