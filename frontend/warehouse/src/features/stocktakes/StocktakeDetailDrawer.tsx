      ...
      {!d ? (q.isLoading ? <LoadingState rows={3} /> : q.isError ? <ErrorState error={q.error} onRetry={() => q.refetch()} /> : null) : (
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-4">
            ...
            {d.status === "draft" && (
              <Button variant="primary" size="sm" disabled={busy} onClick={() => m.start.mutate({ id: d.id, expectedVersion: ev() }, { onSuccess: () => navigate(`/stocktakes/${d.id}/count`), onError })}><Play className="h-4 w-4" /> بدء العد</Button>
            )}
            {d.status === "counting" && (
              <>
                <Button variant="primary" size="sm" onClick={() => navigate(`/stocktakes/${d.id}/count`)}><ClipboardCheck className="h-4 w-4" /> متابعة العد</Button>
