...
import { Money, DateCell, StatusPill } from "@/features/_shared/ui";
...
export function OrderDetailPage() {
  ...
  return (
    <div>
      <Link to="/orders" className="mb-3 inline-flex items-center gap-1 text-sm font-bold text-slate-500 hover:text-teal-700"><ArrowRight className="h-4 w-4" /> رجوع للأوامر</Link>
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            {o.status === "draft" && canConfirm && <Button onClick={() => confirm.mutate()} disabled={busy}><CheckCircle2 className="ml-1 h-4 w-4" /> تأكيد</Button>}
            {o.status === "confirmed" && canFulfill && <Button onClick={() => fulfill.mutate()} disabled={busy}><PackageCheck className="ml-1 h-4 w-4" /> تنفيذ</Button>}
            {o.status === "fulfilled" && canInvoice && <Button onClick={() => invoice.mutate()} disabled={busy}><FileText className="ml-1 h-4 w-4" /> تفويتر</Button>}
            {o.status === "invoiced" && o.invoice_id && <Link to={`/invoices/${o.invoice_id}`}><Button variant="secondary">عرض الفاتورة</Button></Link>}
            {(o.status === "draft" || o.status === "confirmed") && <Button variant="ghost" onClick={() => cancel.mutate()} disabled={busy}><XCircle className="ml-1 h-4 w-4" /> إلغاء</Button>}
          </div>
        }
      />
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Info label="الحالة" value={<StatusPill status={o.status} />} />
        <Info label="التاريخ" value={<DateCell value={o.order_date} />} />
        <Info label="الإجمالي" value={<Money value={o.total_amount} />} />
        <Info label="الفاتورة" value={o.invoice_id ? <Link to={`/invoices/${o.invoice_id}`} className="text-xs text-teal-700 hover:underline">عرض</Link> : "—"} />
      </div>
