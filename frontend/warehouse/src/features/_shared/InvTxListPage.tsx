...
import { invTxStatusToLabel, INVTX_STATUS_OPTIONS } from "@/lib/status-labels";
import { useInvTxList } from "@/lib/hooks/useInventoryTx";
  ...
  return (
    <div>
      <PageHeader
        ...
        action={canCreate ? (<Button variant="primary" onClick={() => navigate(`${config.routeBase}/new`)}><Plus className="h-4 w-4" /> {config.newLabel}</Button>) : null}
      />
      ...
      <section className="mt-4">
        {isLoading ? (
          ...
          <EmptyState title="لا توجد مستندات مطابقة" body={q || status || warehouseId ? "جرّب تعديل عوامل التصفية." : `ابدأ بإنشاء ${config.newLabel}.`} action={canCreate ? <Button onClick={() => navigate(`${config.routeBase}/new`)}><Plus className="h-4 w-4" /> {config.newLabel}</Button> : undefined} />
        ) : (
          <>
