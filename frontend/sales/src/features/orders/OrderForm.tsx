...
interface Values { orderDate: string; lines: LineForm[] }
...
export function OrderForm({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { register, control, handleSubmit, formState: { errors } } = useForm<Values>({
    defaultValues: { orderDate: today, lines: [{ description: "", enteredQty: 1, unitPrice: 0, discount: 0, vatCategory: "S" }] },
  });
  const { fields, append, remove } = useFieldArray({ control, name: "lines" });

  const mutation = useMutation({
    mutationFn: (v: Values) => o2cApi.createOrder({
      customerId: customer?.id, customerName: customer?.name,
      orderDate: v.orderDate, isCreditSale: credit,
    ...
    <Drawer
      ...
      footer={
        <div className="flex items-center justify-end gap-2">
          ...
          <Button onClick={handleSubmit((v) => mutation.mutate(v))} disabled={mutation.isPending}>{mutation.isPending ? "جارٍ الحفظ…" : "حفظ كمسودة"}</Button>
        </div>
      }
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="rounded-2xl border border-slate-200">
          <div className="flex items-center justify-between border-b border-slate-100 px-3 py-2">
            ...
            <Button size="sm" variant="secondary" onClick={() => append({ description: "", enteredQty: 1, unitPrice: 0, discount: 0, vatCategory: "S" })}><Plus className="ml-1 h-4 w-4" /> سطر</Button>
          </div>
