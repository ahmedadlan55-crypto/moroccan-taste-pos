...
// THE SECTION CATALOG
// ────────────────────────────────────────────────────────────────────────
// Mirrors the canonical rows in `statement_sections`: the original 29 rows
// from 0028_coa_metadata.sql plus the 12 missing canonical sections added by
// 0030_statement_sections_gaps.sql. The five historical spellings are kept as
// explicit aliases below (and 0030 also keeps compatible catalog rows for
// them), so existing accounts classify without falling back to a code guess.
//
// `catalogGaps()` remains an executable invariant: after migration 0030 it
// must be empty. A future classifier section that is not backed by a migration
// will immediately surface there and fail the classification gate.
// ════════════════════════════════════════════════════════════════════════
...
const SECTION_CATALOG = [
  ...
  S('other_current_asset', 'balance_sheet', 'currentAssets', 'أصول متداولة أخرى', 'Other current assets', 'debit', 0, 65,
    { bsGroup: ['currentAssets', 'otherCA'], cfBucket: 'otherCurrentAssets' }),
  ...
  S('rou', 'balance_sheet', 'nonCurrentAssets', 'حق استخدام الأصول (IFRS 16)', 'Right-of-use assets', 'debit', 0, 75,
    { bsGroup: ['nonCurrentAssets', 'rou'], cfBucket: 'fixedAssets' }),
  ...
  S('intangibles', 'balance_sheet', 'nonCurrentAssets', 'الأصول غير الملموسة', 'Intangible assets', 'debit', 0, 90,
    { bsGroup: ['nonCurrentAssets', 'intangibles'], cfBucket: 'fixedAssets' }),
  ...
  S('net_vat', 'balance_sheet', 'currentLiabilities', 'صافي ضريبة القيمة المضافة', 'Net VAT', 'credit', 0, 145,
    { bsGroup: ['currentLiab', 'netVat'], cfBucket: 'otherCurrentLiabilities' }),
  ...
  // ── Income statement ──────────────────────────────────────────────────
  S('revenue', 'income_statement', 'revenue', 'الإيرادات', 'Revenue', 'credit', 0, 300,
  ...
  S('stock_variance', 'income_statement', 'cogs', 'فروقات الجرد', 'Stock variances', 'debit', 0, 430,
    { incomeBucket: 'cogs' }),
  S('opex', 'income_statement', 'opex', 'مصروفات تشغيلية', 'Operating expenses', 'debit', 0, 500,
    { incomeBucket: 'opex' }),
...
/** Canonical classifier ids that are missing from statement_sections. */
...
const REPORTABLE_ACCOUNT_SQL = (alias) =>
  `(${alias}.is_active = 1 OR COALESCE(${alias}.status, 'active') IN ('blocked','archived'))`;
...
function _legacyEquitySection(c) {
  ...
  // Historical 343 "zakat reserve" is an equity reserve. Zakat payable is a
  // current liability and uses the explicit stored section `zakat`.
  if (c.startsWith('343')) return 'reserves';
