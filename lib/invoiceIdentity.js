...
const RECEIPT_SETTING_KEYS = Object.freeze([
  ...
  // A4 tax-invoice layout choices (buyer block, bank details, signature,
  // terms). A print preference like paper width — NOT identity — so it
  // stays out of the snapshot and its hash.
  'InvoiceA4Options',
]);
...
function emptyIdentity() {
  return {
    ...
    language: 'ar',
    branchName: '', branchNameEn: '', branchCompanyName: '', brandName: '',
  };
}
...
async function resolveIdentity(db, scope) {
  ...
  if (scope && scope.branchId) {
    try {
      const [b] = await db.query(
        'SELECT id, name, name_en, location, company_name, company_id FROM branches WHERE id = ? LIMIT 1',
        [scope.branchId]
      );
      if (b.length) branch = b[0];
    } catch (_) {
      // branches.name_en is additive (bilingual-i18n-images, Owner A) and may
      // not exist yet on this deploy — retry without it rather than losing
      // the WHOLE branch identity (branchName/address/company_name) to one
      // missing bilingual column.
      try {
        const [b] = await db.query(
        ...
        if (b.length) branch = b[0];
      } catch (_) { /* older schema without these columns at all */ }
    }
  }
  ...
  if (branch) {
    ...
    set('branchNameEn', branch.name_en, 'branches.name_en');
...
/**
 * The A4 tax-invoice layout options (settings.InvoiceA4Options JSON).
 * Defaults are what a professional B2B tax invoice needs: buyer block and
 * signature lines on, bank details off until the owner enters them.
 */
function a4Options(raw) {
  const defaults = { showBuyer: true, showSignature: true, showBank: false, bankDetails: '', terms: '' };
  if (!raw) return defaults;
  try {
    const parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
    const out = { ...defaults };
    for (const k of ['showBuyer', 'showSignature', 'showBank']) {
      if (parsed && Object.prototype.hasOwnProperty.call(parsed, k)) out[k] = isTruthy(parsed[k]);
    }
    for (const k of ['bankDetails', 'terms']) {
      if (parsed && typeof parsed[k] === 'string') out[k] = parsed[k].slice(0, 600);
    }
    return out;
  } catch (_) {
    return defaults;
...
/** Read the A4 options straight from settings (for a route that has no identity resolve at hand). */
async function loadA4Options(db) {
  try {
    const [rows] = await db.query("SELECT setting_value FROM settings WHERE setting_key = 'InvoiceA4Options' LIMIT 1");
    return a4Options(rows.length ? rows[0].setting_value : null);
  } catch (_) {
    return a4Options(null);
  }
}
