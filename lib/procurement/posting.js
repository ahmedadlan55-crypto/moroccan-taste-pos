...
const PPV_CODE = gl.CORE_ACCOUNTS.PPV.code;
...
async function postReceipt(conn, { grn, net, warehouseId, valueByWarehouse }) {
  ...
  if (warehouseEntries.length && warehouseTotal !== n) {
    throw err('GL_POSTING_FAILED', `قيمة المستودعات (${warehouseTotal}) لا تساوي قيمة الاستلام (${n})`);
  }
  const inventoryEntries = warehouseEntries.length
    ...
    : [{ accountCode: inventoryCode, debit: n, credit: 0,
...
async function postPayment(conn, { payment, amount }) {
  ...
  const creditCode = method === 'cash' ? gl.CORE_ACCOUNTS.CASH.code : gl.CORE_ACCOUNTS.BANK.code;
