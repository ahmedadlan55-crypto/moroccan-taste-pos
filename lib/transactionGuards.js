...
async function guardDeveloper(req, res, next) {
  try {
    // V5.7.2: /workflow/* is in the public-path list (auth not enforced globally),
    // so req.user might be empty. Decode the JWT here ourselves.
    if (!req.user) {
...
async function guardAdmin(req, res, next) {
  try {
    // /workflow/* is in the public-path list (auth not enforced globally),
    // so req.user might be empty. Decode the JWT here ourselves.
    if (!req.user) {
      const authHeader = req.headers && req.headers.authorization;
      if (authHeader && authHeader.startsWith('Bearer ')) {
