 ...
 * ── COA_BOOT_REPAIR ──────────────────────────────────────────────────────
 *
 * "The owner runs nothing" was right about the SCRIPT and wrong about the
 * TIMING. Between them these three steps rename a control account, deactivate
 * another, create ~25 accounts through ensureCoreAccounts, and POST A REAL
 * JOURNAL — a document with a number, a date and a balance that lands in the
 * ledger and on the statements — all as a side effect of a process starting.
 * Nobody chose the posting date; a restart did.
 *
 * So the steps are kept exactly as they are, and gated on the same flag as
 * the chart-of-accounts repairs in server.js:
 *
 *   COA_BOOT_REPAIR=1  → unchanged: rename, reclassify, post, backfill.
 *   anything else      → each step runs its DETECTION queries and reports
 *                        what it WOULD do (the balance, the split by
 *                        supplier, the unallocated remainder, the row
 *                        counts), and writes nothing.
 *
 * The diagnostic is not a lesser version of the repair — it is the review
 * step that was missing. An operator reads the planned journal in the boot
 * log, agrees with it, and only then sets the flag.
 */

const acctDate = require('../accountingDate');

const AP_CODE = '211100';
const AP_NAME = 'ذمم دائنة';
const LEGACY_AP_CODE = '2100';
const UNALLOCATED_CODE = '219900';
const UNALLOCATED_NAME = 'ذمم دائنة — غير موزَّعة';

// Read per call rather than captured at require() time: server.js loads this
// module lazily at boot, and an operator flipping the variable should not have
// to reason about module cache order. Strict `=== '1'` — see server.js.
...
async function nameControlAccount(db, log) {
  ...
  // COA_BOOT_REPAIR — the rename is safe for posted history but it is still a
  // change to a control account's identity, and identity is the operator's
  // call, not the restart's.
  if (!coaBootRepairEnabled()) {
    if (String(rows[0].name_ar || '') !== AP_NAME) {
      log('[coa-drift] party — would rename ' + AP_CODE + ' «' + rows[0].name_ar + '» → «' + AP_NAME +
        '» · SKIPPED (set COA_BOOT_REPAIR=1 to apply)');
    }
    const [[dim]] = await db.query(
      "SELECT COUNT(*) AS n FROM gl_accounts WHERE code = ? AND (dim_required IS NULL OR dim_required = '')",
      [AP_CODE]);
    if (Number(dim.n) > 0) {
      log('[coa-drift] party — would mark ' + AP_CODE + " party-required (dim_required = ['party:vendor'])" +
        ' · SKIPPED (set COA_BOOT_REPAIR=1 to apply)');
    }
    return false;
  } else {
...
async function reclassifyLegacyAp(db, glPosting, log) {
  ...
  if (!Number(bal.n) || net === 0) {
    ...
    if (!coaBootRepairEnabled()) {
      const [[act]] = await db.query(
        'SELECT COUNT(*) AS n FROM gl_accounts WHERE code = ? AND is_active = 1', [LEGACY_AP_CODE]);
      if (Number(act.n) > 0) {
        log('[coa-drift] party — ' + LEGACY_AP_CODE + ' is provably empty; would retire it' +
          ' · SKIPPED (set COA_BOOT_REPAIR=1 to apply)');
      }
      return { moved: 0, reason: 'empty — retirement reported, not applied' };
    } else {
  ...
  // COA_BOOT_REPAIR — everything from here down WRITES: ensureCoreAccounts
  // creates ~25 accounts, the 2109 bucket is INSERTed, a real journal is
  // posted, and the legacy account is retired. The plan is fully computed and
  // balance-checked above, so reporting it costs nothing and is exactly what
  // an operator needs in order to approve it. Deliberately placed AFTER the
  // balance check: a diagnostic that advertises an unbalanced journal as
  // "what would happen" would be advertising something that never could.
  if (!coaBootRepairEnabled()) {
    const unal = Math.round(unallocated * 100) / 100;
    log('[coa-drift] party — would post a reclassification journal (' + acctDate.journalDate() + '): Dr ' +
      LEGACY_AP_CODE + ' ' + (dr / 100) + ' → Cr ' + AP_CODE + ' across ' + perSupplier.size +
      ' identified supplier(s)' + (Math.abs(unallocated) > 0.004 ? ' + ' + unal + ' to ' + UNALLOCATED_CODE : '') +
      ', ensure the core accounts exist, and retire ' + LEGACY_AP_CODE +
      ' · SKIPPED (set COA_BOOT_REPAIR=1 to apply)');
    return { moved: 0, planned: perSupplier.size, unallocated: unal,
      reason: 'diagnostic — COA_BOOT_REPAIR off' };
  } else {
...
async function backfillParty(db, log) {
  ...
  // COA_BOOT_REPAIR — these lanes write to POSTED journal lines. The
  // diagnostic deliberately does NOT re-express each UPDATE ... JOIN as a
  // SELECT COUNT: that would mean maintaining a second copy of every join,
  // free to drift from the one that actually runs — which is precisely the
  // failure mode that let the `ap_invoices` typo report success while filling
  // nothing. Instead it reports the one number that is both true and
  // load-bearing: how many payable lines still carry no counterparty.
  let total = 0;
  if (!coaBootRepairEnabled()) {
    const [[pending]] = await db.query(
      'SELECT COUNT(*) AS n FROM gl_entries WHERE account_code = ? AND party_id IS NULL', [AP_CODE]);
    if (Number(pending.n) > 0) {
      log('[coa-drift] party — ' + pending.n + ' payable line(s) on ' + AP_CODE +
        ' carry no party; would backfill from ' + SOURCES.length +
        ' document source(s) · SKIPPED (set COA_BOOT_REPAIR=1 to apply)');
    }
  } else {
