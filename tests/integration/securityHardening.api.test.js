...
(async () => {
  ...
  try {
    ...
    // 1) Scoped CSP — the standalone /warehouse SPA is RETIRED (Closure Sprint
    // v2): both /warehouse and /warehouse-v2 302-redirect into the unified app
    // (/app/inventory), and the strict CSP now rides on the /app mount.
    const csp = await req('GET', '/app/');
    const cspHeader = csp.headers && csp.headers['content-security-policy'];
    check('/app carries a Content-Security-Policy header', !!cspHeader, cspHeader);
    ...
    check('/warehouse-v2 → redirect into the unified app (/app/inventory)', alias.status === 302 && String((alias.headers && alias.headers.location) || '').indexOf('/app/inventory') === 0, { status: alias.status, location: alias.headers && alias.headers.location });
