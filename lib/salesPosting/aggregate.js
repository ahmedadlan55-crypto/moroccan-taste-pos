...
// Invoice drill-down remains available inside every batch. Invoice-by-invoice
// posting is intentionally not an option because it recreates the GL-volume
// problem this subsystem exists to solve.
const GRANULARITIES = Object.freeze(['daily', 'monthly']);
...
function bucketKeyOf(row, granularity) {
  ...
  // GL periods follow the Riyadh calendar date stamped on the tax invoice.
  // business_day remains an analytics dimension only (a 01:00 sale may belong
  // to the previous trading night but must not cross accounting periods).
  if (granularity === 'monthly') return ['monthly', monthOf(row.calendar_date), brand, branch].join('|');
  return ['daily', dayOf(row.calendar_date), brand, branch].join('|');
}

/** The human-facing label for a bucket. */
function bucketLabelOf(row, granularity) {
  if (granularity === 'monthly') return monthOf(row.calendar_date);
  return dayOf(row.calendar_date);
}
...
function buildLegs(bucket, accounts) {
  ...
  const commissionExpense = new Map();
  const commissionPayable = new Map();
  ...
  for (const r of bucket.rows) {
    ...
    if (pays.length) {
      ...
      // Never invent cash. Missing split data is a reconciliation defect and
      // must block the batch until the source event is repaired.
      warnings.push('PAYMENT_SPLIT_MISSING: ' + r.source_type + ' ' + r.source_id);
    }
    ...
    // Aggregate platform commission with the sale instead of creating a
    // second journal for every invoice. Amounts are already signed.
    const commissions = Array.isArray(p.commissions) ? p.commissions : [];
    for (const fee of commissions) {
      byCode(commissionExpense, fee.expenseCode || accounts.platformCommission, C(fee.amount));
      byCode(commissionPayable, fee.payableCode || accounts.platformPayable, C(fee.amount));
    }
  }
  ...
  for (const [code, cents] of commissionExpense) push(code, cents, 'debit', null, 'commission');
  for (const [code, cents] of commissionPayable) push(code, cents, 'credit', null, 'commission-payable');
...
function planBatches(rows, granularity, accounts) {
  ...
  return buckets.map((b) => {
    ...
    return {
      ...
      // Carry the immutable per-event amounts into batch_items. These are the
      // audit drill-down for the aggregate journal; writing zero for net/tax
      // made a correctly balanced batch present false invoice detail.
      sources: b.rows.map((r) => ({
        id: r.id, type: r.source_type, sourceId: r.source_id,
        invoiceNumber: r.invoice_number,
        net: R(C(r.net_amount)),
        tax: R(C(r.tax_amount)),
        gross: R(C(r.gross_amount)),
        cogs: R(C(r.cogs_amount)),
      })),
      legs: built.legs,
      balanced: built.balanced,
      warnings: built.warnings,
      postable: built.balanced && !built.warnings.some((w) =>
        w.startsWith('PAYMENT_MISMATCH') || w.startsWith('PAYMENT_SPLIT_MISSING')),
    };
  });
}
