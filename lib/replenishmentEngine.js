...
// isActive defaults to true when omitted (back-compat). An INACTIVE item is never
// a buy signal — if it still holds stock it surfaces as 'inactive_balance' (a
// balance to liquidate), never 'reorder'/'critical' (Phase 4B — close 4A).
function reorderStatus(onHand, rule, isActive) {
  const oh = nn(onHand);
  if (oh < 0) return 'negative';
  if (isActive === false) return oh > 0 ? 'inactive_balance' : 'ok';
...
function recommendedQty(onHand, rule, isActive) {
  if (isActive === false) return 0; // Phase 4B — an inactive item never recommends a buy
  const oh = nn(onHand);
...
function computeReplenishment(o) {
  ...
  const isActive = o.isActive !== false; // default active when omitted (back-compat)
  return {
    onHand,
    available: onHand, // reserved not tracked
    avgDailyOutbound: round3(ado),
    daysOfCover: cover,
    reorderStatus: reorderStatus(onHand, rule, isActive),
    recommendedQty: recommendedQty(onHand, rule, isActive),
