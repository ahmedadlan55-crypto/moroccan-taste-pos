...
const ACCOUNT_COLUMNS =
  ...
  'is_postable, is_control, cash_flow_activity, status, version, ' +
...
async function childCount(conn, id) {
  const r = await rows(conn, 'SELECT COUNT(*) AS n FROM gl_accounts WHERE parent_id = ?', [id]);
...
async function findByCode(conn, code, companyId, excludeId) {
  const r = await rows(
    conn,
    'SELECT id, code FROM gl_accounts WHERE code = ? AND ' +
      '(company_id <=> ? OR ? IS NULL) AND (id <> ? OR ? IS NULL) LIMIT 1',
    [code, companyId == null ? null : companyId, companyId == null ? null : companyId,
...
async function subtree(conn, id) {
  ...
  while (frontier.length) {
    ...
    const kids = await rows(
      conn,
      'SELECT id, code, name_ar, parent_id FROM gl_accounts WHERE parent_id IN (' + placeholders + ')',
      frontier
    );
    const next = [];
    for (const k of kids) {
...
async function resolveParentContext(conn, args) {
  ...
  const parent = await loadAccount(conn, parentId, { lock });
  ...
  // A posting leaf may not be a parent. It becomes a folder instead — unless
  // it already carries journal entries, in which case promoting it would hide
  // real postings behind a header account and break every "postings only at
  // leaves" invariant in the reports.
...
function normalizeInput(input) {
  ...
  return {
    ...
    code,
    nameAr,
    nameEn: src.nameEn == null ? '' : String(src.nameEn),
    type,
...
async function createAccountTx(conn, input, ctx) {
  ...
  const companyId = parentCtx.parent ? parentCtx.parent.company_id : (c.companyId || null);
  ...
  await conn.query(
    ...
    'is_active, status, company_id, normal_balance, is_postable, version, created_by, updated_by, updated_at) ' +
    'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?,NOW())',
  ...
  await coaTree.recomputeLevels(conn, { rootId: id });
...
async function updateAccountTx(conn, id, input, ctx) {
  ...
  await conn.query(
    ...
    'is_folder = ?, is_active = ?, status = ?, normal_balance = ? WHERE id = ?',
    [data.code, data.nameAr, data.nameEn, data.type, data.parentId || null,
     isFolder, life.isActive, life.status, normalBalance, id]
  );
  ...
  await audit(conn, 'coa.update', id, actor, {
    before,
    after: {
      code: data.code, nameAr: data.nameAr, nameEn: data.nameEn, type: data.type,
...
async function previewMoveTx(conn, id, newParentId, args) {
  ...
  try {
    parentCtx = await resolveParentContext(conn, {
      parentId: newParentId || null, movingId: id, type: acc.type, height, lock: false,
    });
  } catch (e) { push(e); }
...
module.exports = {
  ...
  MAX_DEPTH, MAX_WALK, ACCOUNT_TYPES, STATUSES, ERROR_STATUS,
