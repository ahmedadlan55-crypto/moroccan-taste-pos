 ...
 */
async function postCreditNote(conn, { ret, net, vat, cost = 0, warehouseId }) {
  const acc = await ensureO2CAccounts(conn);
  ...
  const entries = [
    { accountCode: acc.revenue.code, debit: n, credit: 0, description: 'عكس إيراد (مرتجع)' },
  ];
  ...
  if (c > 0) {
    await _post(conn, {
      ...
      entries: [
        { accountCode: acc.inventory.code, debit: c, credit: 0, warehouseId: warehouseId || ret.warehouse_id, description: 'مخزون' },
        { accountCode: acc.cogs.code, debit: 0, credit: c, description: 'عكس تكلفة المبيعات' },
      ],
    });
  }
  return jId;
