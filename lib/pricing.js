...
function splitLinePrice(price, isInclusive, ratePct) {
  ...
  // WHOLE-RIYAL SHELF PRICING (v8.2) — owner requirement 3 at the top of this
  // file, applied where it actually holds: the PER-UNIT customer-facing price.
  //
  // v8.1 removed the whole-riyal round because it was applied to the INVOICE
  // TOTAL, and pushing that delta onto the net side made the invoice report a
  // net the owner never priced. Rounding the UNIT instead has neither problem:
  // net and vat are derived FROM the rounded gross, so `net + vat === gross`
  // exactly (the journal invariant and ZATCA both need that), and a customer
  // quoted "35 each" pays exactly 70 for two.
  //
  // This is the byte-for-byte twin of wholeUnitGross in lib/posOrderMachine.js
  // and frontend/pos/src/lib/cartMath.ts. Change one without the others and the
  // reconciliation guard rejects every sale.
  // _round2 FIRST. 50 × 1.15 is 57.49999999999999 in IEEE-754, and rounding
  // that straight to a whole riyal gives 57 — a full riyal below the 58 every
  // human and every calculator gets. Snapping to halalas first restores the
  // exact 57.50 the arithmetic means, and only then do we round to the riyal.
  const grossPrecise = p <= 0
    ? 0
    : Math.max(1, roundToWhole(_round2(isInclusive ? p : p * (1 + rate / 100))));
  const net = rate > 0 ? grossPrecise / (1 + rate / 100) : grossPrecise;
  const vat = grossPrecise - net;
