...
import { useUnits, useItemCategories, useItemDetail, useItemMutations } from "@/lib/hooks/useItems";
import { createItemInput, updateItemInput } from "@/lib/schemas/item.schema";
...
export function ItemWizard() {
  ...
  function submit() {
    ...
    if (editId) m.update.mutate({ id: editId, input: { ...payload, expectedVersion: edit.data?.version ?? 0 } }, { onSuccess: () => navigate(`/items?view=${editId}`), onError: onErr });
    else m.create.mutate(payload, { onSuccess: (res) => navigate(`/items?view=${res.id ?? ""}`), onError: onErr });
  }
  ...
  return (
    <div>
      <PageHeader eyebrow="كتالوج الأصناف" title={editId ? `تعديل صنف — ${edit.data?.name ?? ""}` : "صنف جديد"} subtitle="بيانات الصنف الأساسية. متوسط تكلفة المستودع (WAC) لا يُحرَّر هنا."
        action={<Button variant="ghost" onClick={() => navigate("/items")}><X className="h-4 w-4" /> إغلاق</Button>} />
