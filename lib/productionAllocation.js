...
async function loadGenealogy(conn, productionOrderId) {
  ...
  // ── pre-ledger orders ───────────────────────────────────────────────────
  // Migration 0027 created this ledger EMPTY, so every order produced before
  // the deploy has no allocation rows. Reading only the ledger made those
  // orders report "no genealogy" — which for a food business reads as "we
  // cannot trace this batch", when in fact the linkage was recorded all along
  // in production_output_lots (still written today, at :169).
  //
  // Fall back to it, flagged. What is trustworthy there is WHICH component lot
  // went into WHICH output lot — that is the recall question. What is not is
  // the per-output quantity split, because the old writer attributed the whole
  // order's consumption to every output lot; that is the defect the ledger
  // exists to fix. So the linkage is returned and `approximate` marks the qty.
  const [legacy] = await conn.query(
    `SELECT pol.*, lc.lot_number AS component_lot_number, lo.lot_number AS output_lot_number,
            i.name AS component_name, lc.item_id AS component_item_id
       FROM production_output_lots pol
       LEFT JOIN inventory_lots lc ON lc.id = pol.component_lot_id
       LEFT JOIN inventory_lots lo ON lo.id = pol.output_lot_id
       LEFT JOIN inv_items i ON i.id = lc.item_id
      WHERE pol.work_order_id=?
      ORDER BY pol.created_at, pol.id`, [productionOrderId]);
  return legacy.map((r) => ({
    outputEventId: null,
    outputLotId: r.output_lot_id,
    outputLotNumber: r.output_lot_number || null,
    producedAt: r.created_at || null,
    componentItemId: r.component_item_id || null,
    componentName: r.component_name || r.component_item_id || null,
    componentLotId: r.component_lot_id,
    componentLotNumber: r.component_lot_number || null,
    issueEventId: null,
    issueLineId: null,
