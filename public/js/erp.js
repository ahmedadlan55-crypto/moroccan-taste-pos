...
function erpNav(sectionId) {
  ...
  if (target) {
    ...
    switch(sectionId) {
      ...
      // ═══ ERP v3 standard reports ═══
      case 'erpRptTrialBalance':   erpLoadTrialBalance(); break;
      case 'erpRptPnL':             erpLoadPnL(); break;
...
// ═══════════════════════════════════════
let _erpSuppliers = [];
let _erpSupBrandsCache = null;
function erpLoadSuppliers() {
  var tbody = document.getElementById('erpSuppliersBody');
  if (tbody) tbody.innerHTML = '<tr><td colspan="9" class="empty-msg"><i class="fas fa-spinner fa-spin"></i> جاري التحميل...</td></tr>';
  // Populate brand filter once
  if (!_erpSupBrandsCache) {
    _erpGet('/erp/brands', function(brs){
      _erpSupBrandsCache = brs || [];
      var sel = document.getElementById('erpSupFBrand');
      if (sel) sel.innerHTML = '<option value="">كل البراندات</option>' + _erpSupBrandsCache.map(function(b){return '<option value="'+b.id+'">'+_woEscapeHtml(b.name||'')+'</option>';}).join('');
    });
  }
  window._apiBridge
    ...
    .withFailureHandler(function(e) {
      if (tbody) tbody.innerHTML = '<tr><td colspan="9" class="empty-msg" style="color:red;">خطأ: ' + e.message + '</td></tr>';
    })
    .getSuppliers();
}
...
function erpRenderSuppliersTable(list) {
  ...
  if (!list || list.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" class="empty-msg">لا يوجد موردين</td></tr>';
    return;
  }
  tbody.innerHTML = list.map(s => {
    var brandChip = s.brandName
      ? `<span class="wo-chip purple"><i class="fas fa-store"></i> ${_woEscapeHtml(s.brandName)}</span>`
...
function erpOpenSupplierModal(data) {
  ...
  _erpGet('/erp/brands', function(brs){
    var brandOpts = '<option value="">— مورد عام (كل البراندات) —</option>' +
      (brs||[]).map(function(b){
        var sel = (d.brandId === b.id) ? ' selected' : '';
        return '<option value="'+b.id+'"'+sel+'>'+_woEscapeHtml(b.name||'')+'</option>';
      }).join('');
    document.getElementById('erpModalBody').innerHTML = `
...
function erpSaveSupplier() {
  const data = {
    ...
    brandId: (document.getElementById('erpSupBrand') && document.getElementById('erpSupBrand').value) || null,
...
let _erpPOBrandsCache = null;
function erpLoadPOs() {
  if (!_erpPOBrandsCache) {
    _erpGet('/erp/brands', function(brs){
      _erpPOBrandsCache = brs || [];
      var sel = document.getElementById('erpPOFBrand');
      if (sel) sel.innerHTML = '<option value="">كل البراندات</option>' + _erpPOBrandsCache.map(function(b){return '<option value="'+b.id+'">'+_woEscapeHtml(b.name||'')+'</option>';}).join('');
    });
  }
  window._apiBridge.withSuccessHandler(function(list) {
    _erpPOAllData = list || [];
...
function erpRenderPOTable(list) {
  ...
  tbody.innerHTML = list.map(function(po) {
    ...
    var brandName = po.brandName || po.brand_name || '';
    var brandChip = brandName
      ? '<span class="wo-chip purple"><i class="fas fa-store"></i> ' + _woEscapeHtml(brandName) + '</span>'
      : '<span class="wo-text-subtle wo-text-caption">عام</span>';
    return '<tr>' +
...
function erpShowPOForm() {
  ...
  _erpPOCart = [];
  erpRenderPOCart();
  // Populate brand + branch selects
  _erpGet('/erp/brands', function(brs){
    var bs = document.getElementById('erpPOBrand');
    if (bs) bs.innerHTML = '<option value="">— عام —</option>' + (brs||[]).map(function(b){return '<option value="'+b.id+'">'+_woEscapeHtml(b.name||'')+'</option>';}).join('');
  });
  _erpGet('/erp/branches-full', function(bnrs){
    var bs = document.getElementById('erpPOBranch');
    if (bs) bs.innerHTML = '<option value="">— الكل —</option>' + (bnrs||[]).map(function(b){return '<option value="'+b.id+'">'+_woEscapeHtml(b.name||'')+'</option>';}).join('');
  });
  // Load data
...
function erpSavePO() {
  ...
  var poData = {
    ...
    brandId: document.getElementById('erpPOBrand')?.value || null,
    branchId: document.getElementById('erpPOBranch')?.value || null,
...
// ─── shared helpers for workflow UI ────────────────────────────
window._wfImpColors = { critical:'#dc2626', high:'#ea580c', medium:'#ca8a04', low:'#16a34a' };
window._wfImpBgs    = { critical:'#fee2e2', high:'#ffedd5', medium:'#fef9c3', low:'#dcfce7' };
window._wfImpLabels = { critical:'عاجل', high:'عالي', medium:'متوسط', low:'منخفض' };
window._wfStatLbl   = { pending:'قيد الانتظار', in_progress:'قيد التنفيذ', approved:'معتمدة', rejected:'مرفوضة', closed:'مغلقة', draft:'مسودة' };
...
function wfInitDashboard() {
  window._apiBridge.withSuccessHandler(function(f) {
    var bSel = document.getElementById('wfDashFBranch');
    var dSel = document.getElementById('wfDashFDept');
    var tSel = document.getElementById('wfDashFType');
    if (!bSel) return;
    bSel.innerHTML = '<option value="">كل الفروع</option>' + (f.branches||[]).map(function(b) { return '<option value="' + b.id + '">' + b.name + '</option>'; }).join('');
    window._wfDashDeptsAll = f.departments || [];
    dSel.innerHTML = '<option value="">كل الأقسام</option>' + window._wfDashDeptsAll.map(function(d) { return '<option value="' + d.id + '" data-branch="' + (d.branchId||'') + '">' + d.name + '</option>'; }).join('');
    tSel.innerHTML = '<option value="">كل الأنواع</option>' + (f.types||[]).map(function(t) { return '<option value="' + t.id + '">' + t.name + '</option>'; }).join('');
    bSel.onchange = function() { wfFilterDeptsByBranch(); wfLoadDashboard(); };
    // Importance summary cards
    var counts = f.counts || {};
    var total = (counts.critical||0) + (counts.high||0) + (counts.medium||0) + (counts.low||0);
    var imp = document.getElementById('wfDashImportance');
    function card(key, icon, labelAr) {
      var c = window._wfImpColors[key], bg = window._wfImpBgs[key], n = counts[key]||0;
      return '<div style="background:linear-gradient(135deg,'+bg+','+bg+'80);border:1px solid '+c+'40;border-radius:14px;padding:14px;display:flex;align-items:center;gap:12px;cursor:pointer;" onclick="document.getElementById(\'wfDashFImp\').value=\''+key+'\';wfLoadDashboard();">' +
        '<div style="width:42px;height:42px;border-radius:12px;background:'+c+';color:#fff;display:flex;align-items:center;justify-content:center;font-size:16px;"><i class="fas '+icon+'"></i></div>' +
        '<div><div style="font-size:11px;color:'+c+';font-weight:800;">'+labelAr+'</div><div style="font-size:24px;font-weight:900;color:#0f172a;">'+n+'</div></div>' +
        '</div>';
    }
    imp.innerHTML =
      '<div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:14px;display:flex;align-items:center;gap:12px;cursor:pointer;" onclick="wfResetDashFilters()">' +
        '<div style="width:42px;height:42px;border-radius:12px;background:#0ea5e9;color:#fff;display:flex;align-items:center;justify-content:center;font-size:16px;"><i class="fas fa-layer-group"></i></div>' +
        '<div><div style="font-size:11px;color:#64748b;font-weight:800;">كل المعاملات المفتوحة</div><div style="font-size:24px;font-weight:900;color:#0f172a;">'+total+'</div></div>' +
      '</div>' +
      card('critical','fa-exclamation-triangle','عاجل (Critical)') +
      card('high','fa-fire','عالي (High)') +
      card('medium','fa-minus','متوسط (Medium)') +
      card('low','fa-leaf','منخفض (Low)');
    wfLoadDashboard();
  }).getWfDashFilters();
}

function wfFilterDeptsByBranch() {
  var bId = document.getElementById('wfDashFBranch').value;
  var dSel = document.getElementById('wfDashFDept');
  var current = dSel.value;
  var filtered = bId ? window._wfDashDeptsAll.filter(function(d){return !d.branchId || d.branchId === bId;}) : window._wfDashDeptsAll;
  dSel.innerHTML = '<option value="">كل الأقسام</option>' + filtered.map(function(d) { return '<option value="' + d.id + '">' + d.name + '</option>'; }).join('');
  if (filtered.some(function(d){return d.id===current;})) dSel.value = current;
}

function wfResetDashFilters() {
  ['wfDashFBranch','wfDashFDept','wfDashFImp','wfDashFType','wfDashFStatus'].forEach(function(id) {
    var el = document.getElementById(id); if (el) el.value = '';
  });
  wfFilterDeptsByBranch();
  wfLoadDashboard();
}
...
function wfLoadDashboard() {
  ...
  var params = {
    branchId: document.getElementById('wfDashFBranch').value || '',
    deptId:   document.getElementById('wfDashFDept').value || '',
    importance: document.getElementById('wfDashFImp').value || '',
    typeId:   document.getElementById('wfDashFType').value || '',
    status:   document.getElementById('wfDashFStatus').value || ''
  };
  window._apiBridge.withSuccessHandler(function(list) {
    if (!list || !list.length) {
      cards.innerHTML = '<div style="text-align:center;padding:40px;color:#94a3b8;grid-column:1/-1;font-style:italic;">لا توجد معاملات مطابقة</div>';
      return;
    }
    cards.innerHTML = list.map(function(t) {
      var c = window._wfImpColors[t.importance]||'#6b7280';
      var bg = window._wfImpBgs[t.importance]||'#f3f4f6';
      var dt = t.createdAt ? new Date(t.createdAt).toLocaleDateString('en-GB') : '';
      return '<div onclick="wfViewTxn(\''+t.id+'\')" style="cursor:pointer;background:#fff;border:2px solid '+c+';border-radius:14px;padding:14px;display:flex;flex-direction:column;gap:8px;transition:transform .15s;box-shadow:0 2px 8px '+c+'20;" onmouseover="this.style.transform=\'translateY(-3px)\';" onmouseout="this.style.transform=\'\';">' +
        '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">' +
          _wfImpBadge(t.importance||'medium') + _wfStatBadge(t.status) +
        '</div>' +
        '<div style="font-family:monospace;font-size:10px;color:#64748b;">'+(t.txnNumber||'')+'</div>' +
        '<div style="font-size:14px;font-weight:800;color:#0f172a;line-height:1.4;max-height:42px;overflow:hidden;">'+(t.title||'')+'</div>' +
        '<div style="font-size:11px;color:#475569;max-height:34px;overflow:hidden;">'+((t.description||'').substring(0,120))+(((t.description||'').length>120)?'…':'')+'</div>' +
        '<div style="border-top:1px dashed #e5e7eb;padding-top:8px;display:flex;flex-direction:column;gap:3px;font-size:11px;">' +
...
function wfCancelOutboxTxn(id) {
  erpConfirm('إلغاء المعاملة', 'هل أنت متأكد من إلغاء المعاملة؟ لن يمكن استرجاعها.', function() {
    loader(true);
    window._apiBridge.withSuccessHandler(function(r) {
      loader(false);
      if (r.success) { showToast('تم الإلغاء'); wfLoadOutbox(); }
      else showToast(r.error, true);
...
function wfLoadOrgTree() {
  var tb = document.getElementById('wfOrgBody');
  if (!tb) return;
  tb.innerHTML = '<tr><td colspan="8" class="empty-msg"><i class="fas fa-spinner fa-spin"></i></td></tr>';
  window._apiBridge.withSuccessHandler(function(list) {
    window._wfOrgEmps = list || [];
    if (!list || !list.length) { tb.innerHTML = '<tr><td colspan="8" class="empty-msg">لا يوجد موظفون</td></tr>'; return; }
    var empsById = {};
    list.forEach(function(e) { empsById[e.id] = e; });
    tb.innerHTML = list.map(function(e) {
      var mgr = e.managerId && empsById[e.managerId] ? empsById[e.managerId].fullName : '—';
      var p = e.permissions || {};
      var chips = [
        ...
        return '<span style="padding:2px 8px;border-radius:6px;font-size:10px;font-weight:700;margin:2px;display:inline-block;'+
          (active ? ('background:'+x[2]+'20;color:'+x[2]) : 'background:#f3f4f6;color:#94a3b8;')+'">'+x[1]+'</span>';
      }).join('');
      return '<tr>' +
        '<td><div style="font-weight:800;">'+(e.fullName||'')+'</div><div style="font-size:10px;color:#94a3b8;">'+(e.username||'—')+'</div></td>' +
        '<td>'+(e.jobTitle||e.positionName||'—')+'</td>' +
        '<td>'+(e.branchName||'—')+'</td>' +
        '<td>'+(e.deptName||'—')+'</td>' +
        '<td><span class="badge badge-blue">L'+(e.workflowLevel||1)+'</span></td>' +
        '<td style="font-size:12px;">'+mgr+'</td>' +
        '<td>'+chips+'</td>' +
        '<td><button class="btn btn-sm btn-primary" onclick="wfEditOrgEmp(\''+e.id+'\')"><i class="fas fa-edit"></i></button></td>' +
      '</tr>';
    }).join('');
  }).getWfOrgTree();
}

function wfEditOrgEmp(id) {
  var emp = (window._wfOrgEmps||[]).find(function(x){return x.id===id;});
  if (!emp) return;
  var others = (window._wfOrgEmps||[]).filter(function(x){return x.id!==id;});
  var mgrOpts = '<option value="">—</option>' + others.map(function(e) {
    return '<option value="'+e.id+'"'+(emp.managerId===e.id?' selected':'')+'>'+(e.fullName||'')+' — '+(e.positionName||e.jobTitle||'')+'</option>';
  }).join('');
  var p = emp.permissions || {};
  function chk(k, l) {
    return '<label style="display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:8px;background:#f8fafc;cursor:pointer;"><input type="checkbox" id="wfOrgPerm_'+k+'"'+(p[k]?' checked':'')+'> '+l+'</label>';
  }
  document.getElementById('erpModalTitle').textContent = 'تعديل صلاحيات الموظف — ' + (emp.fullName||'');
  document.getElementById('erpModalBody').innerHTML =
    '<div style="padding:10px;background:#eff6ff;border-radius:10px;margin-bottom:12px;font-size:13px;">' +
      '<div><b>'+(emp.fullName||'')+'</b> — '+(emp.jobTitle||emp.positionName||'')+'</div>' +
      '<div style="font-size:11px;color:#64748b;">'+(emp.branchName||'—')+' • '+(emp.deptName||'—')+'</div>' +
    '</div>' +
    '<div class="form-row"><label>المدير المباشر</label><select class="form-control" id="wfOrgMgr">'+mgrOpts+'</select></div>' +
    '<div class="form-row"><label>المستوى الإداري (Level)</label><select class="form-control" id="wfOrgLevel">' +
      [1,2,3,4,5,6,7].map(function(n){return '<option value="'+n+'"'+(n===emp.workflowLevel?' selected':'')+'>Level '+n+'</option>';}).join('') +
    '</select></div>' +
    '<div class="form-row"><label>الصلاحيات في المعاملات</label><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">' +
      chk('create','إنشاء') + chk('approve','موافقة') + chk('reject','رفض') +
      chk('return','إرجاع') + chk('forward','تحويل') + chk('close','إغلاق') +
    '</div></div>';
  document.getElementById('erpModalSaveBtn').onclick = function() {
    var perms = {};
    ['create','approve','reject','return','forward','close'].forEach(function(k) {
      perms[k] = document.getElementById('wfOrgPerm_'+k).checked;
    });
    loader(true);
    window._apiBridge.withSuccessHandler(function(r) {
      loader(false);
      if (r.success) { showToast('تم الحفظ'); erpCloseModal(); wfLoadOrgTree(); }
      else showToast(r.error, true);
    }).updateWfOrg(id, {
      managerId: document.getElementById('wfOrgMgr').value || null,
      workflowLevel: Number(document.getElementById('wfOrgLevel').value) || 1,
      permissions: perms
    });
  };
  document.getElementById('erpModal').classList.remove('hidden');
...
// ═════════════════════════════════════════════════════════════════════════════
// ERP v3 — STANDARD REPORTS (Trial Balance, P&L, Balance Sheet, Cash Flow,
// Profitability, Inventory Valuation, Sales Analytics, Waste Analytics, Royalty)
// Plus admin CRUD for: Companies, Price Lists, BOM, Waste Entries, Royalty
// Runs, POS Terminals, Item Categories.
// ═════════════════════════════════════════════════════════════════════════════
...
function _erpToken() { return localStorage.getItem('pos_token'); }
function _erpGet(path, cb) {
  fetch('/api' + path, { headers: { 'Authorization': 'Bearer ' + _erpToken() } })
    .then(function(r) { return r.json(); })
    .then(cb)
    .catch(function(err) { cb({ success: false, error: err.message }); });
}
function _erpPost(path, body, cb) {
  fetch('/api' + path, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + _erpToken() },
    body: JSON.stringify(body)
  }).then(function(r){return r.json();}).then(cb).catch(function(err){ cb({success:false, error:err.message}); });
}
function _erpDelete(path, cb) {
  fetch('/api' + path, {
    method: 'DELETE',
    headers: { 'Authorization': 'Bearer ' + _erpToken() }
  }).then(function(r){return r.json();}).then(cb).catch(function(err){ cb({success:false, error:err.message}); });
}
...
function erpLoadInventoryValuation() {
  ...
  _erpGet('/erp/warehouses-list', function(whs) {
    ...
    var el = document.getElementById('ivWh'); if (el) el.innerHTML = opts;
  });
  var wh = (document.getElementById('ivWh')||{}).value || '';
...
function erpLoadWasteAnalytics() {
  _erpPopulateBranchOptions(['waBranch']);
  _erpPopulateBrandOptions(['waBrand']);
  var from = (document.getElementById('waFrom')||{}).value || '';
  var to   = (document.getElementById('waTo')||{}).value || '';
  var branch = (document.getElementById('waBranch')||{}).value || '';
  var brand  = (document.getElementById('waBrand')||{}).value || '';
  var qs = new URLSearchParams({ from, to, branch, brand }).toString();
  _erpGet('/erp/reports/waste-analytics?' + qs, function(r) {
    if (!r.success) { showToast(r.error||'خطأ', true); return; }
    var s = r.summary || {};
    document.getElementById('waSummary').innerHTML =
      _erpKpi('إجمالي تكلفة الهدر', _erpFmt(s.totalWasteCost), '#ef4444') +
      _erpKpi('إجمالي المبيعات', _erpFmt(s.totalSales), '#16a34a') +
      _erpKpi('الهدر % من المبيعات', (s.wastePctOfSales||0)+'%', (s.wastePctOfSales||0)>5?'#ef4444':'#f59e0b');
    document.getElementById('waTopBody').innerHTML = (r.topItems||[]).map(function(d){return '<tr><td>'+(d.itemName||'')+'</td><td>'+_erpFmt(d.totalQty)+'</td><td>'+_erpFmt(d.totalCost)+'</td></tr>';}).join('') || '<tr><td colspan="3" class="empty-msg">—</td></tr>';
    var reasonMap = {expired:'منتهي الصلاحية',damaged:'تالف',spill:'انسكاب',prep_loss:'خسارة تحضير',customer_return:'إرجاع عميل',other:'أخرى'};
    document.getElementById('waReasonBody').innerHTML = (r.byReason||[]).map(function(d){return '<tr><td>'+(reasonMap[d.reason]||d.reason||'')+'</td><td>'+d.count+'</td><td>'+_erpFmt(d.totalCost)+'</td></tr>';}).join('') || '<tr><td colspan="3" class="empty-msg">—</td></tr>';
  });
}

// ─── Royalty Reconciliation ───
function erpLoadRoyaltyRecon() {
  _erpPopulateBrandOptions(['rrBrand']);
  var year = (document.getElementById('rrYear')||{}).value || new Date().getFullYear();
  if (!document.getElementById('rrYear').value) document.getElementById('rrYear').value = year;
  var brand = (document.getElementById('rrBrand')||{}).value || '';
  var qs = new URLSearchParams({ year, brand }).toString();
  _erpGet('/erp/reports/royalty-reconciliation?' + qs, function(r) {
    if (!r.success) { showToast(r.error||'خطأ', true); return; }
    var t = r.totals || {};
    ...
    var rows = r.rows || [];
    var statMap = {draft:'مسودة',approved:'معتمد',invoiced:'مفوتر',paid:'مدفوع',cancelled:'ملغى'};
    document.getElementById('rrBody').innerHTML = rows.length ? rows.map(function(x){
      return '<tr><td>'+(x.periodStart||'').slice(0,10)+' → '+(x.periodEnd||'').slice(0,10)+'</td>'+
        '<td>'+(x.brandName||'')+'</td>'+
        '<td>'+_erpFmt(x.grossSales)+'</td>'+
        '<td>'+_erpFmt(x.netSales)+'</td>'+
        '<td style="font-weight:700;">'+_erpFmt(x.royaltyAmount)+'</td>'+
        '<td style="color:#16a34a;">'+_erpFmt(x.paid)+'</td>'+
        '<td style="color:'+(x.outstanding>0?'#ef4444':'#16a34a')+';font-weight:700;">'+_erpFmt(x.outstanding)+'</td>'+
        '<td><span class="badge badge-blue">'+(statMap[x.status]||x.status)+'</span></td></tr>';
    }).join('') : '<tr><td colspan="8" class="empty-msg">لا توجد بيانات لهذه السنة</td></tr>';
  });
}
...
// ─── Companies ───
function erpLoadCompanies() {
  document.getElementById('companiesBody').innerHTML = '<tr><td colspan="7" class="empty-msg"><i class="fas fa-spinner fa-spin"></i></td></tr>';
  _erpGet('/erp/companies', function(list) {
    if (!Array.isArray(list) || !list.length) { document.getElementById('companiesBody').innerHTML = '<tr><td colspan="7" class="empty-msg">لا توجد شركات</td></tr>'; return; }
    document.getElementById('companiesBody').innerHTML = list.map(function(c){
      return '<tr><td><code>'+c.id+'</code></td><td style="font-weight:700;">'+c.name+'</td><td>'+(c.taxNumber||'—')+'</td><td>'+(c.crNumber||'—')+'</td><td>'+(c.city||'—')+'</td><td>'+(c.baseCurrency||'SAR')+'</td>'+
        '<td><button class="btn btn-sm btn-primary" onclick="erpOpenCompanyModal(\''+c.id+'\')"><i class="fas fa-edit"></i></button></td></tr>';
    }).join('');
  });
}
function erpOpenCompanyModal(id) {
  var doRender = function(d) {
    d = d || {};
    document.getElementById('erpModalTitle').textContent = d.id ? ('تعديل شركة — '+d.name) : 'شركة جديدة';
    document.getElementById('erpModalBody').innerHTML =
      '<input type="hidden" id="coId" value="'+(d.id||'')+'">'+
      '<div class="form-row"><label>الاسم *</label><input class="form-control" id="coName" value="'+(d.name||'').replace(/"/g,'&quot;')+'"></div>'+
      '<div class="form-row"><label>الاسم القانوني</label><input class="form-control" id="coLegal" value="'+(d.legalName||'').replace(/"/g,'&quot;')+'"></div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;"><div class="form-row"><label>الرقم الضريبي</label><input class="form-control" id="coTax" value="'+(d.taxNumber||'')+'"></div><div class="form-row"><label>السجل التجاري</label><input class="form-control" id="coCR" value="'+(d.crNumber||'')+'"></div></div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;"><div class="form-row"><label>الدولة</label><input class="form-control" id="coCountry" value="'+(d.country||'SA')+'"></div><div class="form-row"><label>المدينة</label><input class="form-control" id="coCity" value="'+(d.city||'')+'"></div><div class="form-row"><label>العملة</label><input class="form-control" id="coCur" value="'+(d.baseCurrency||'SAR')+'"></div></div>'+
      '<div class="form-row"><label>بداية السنة المالية</label><input type="date" class="form-control" id="coFY" value="'+(d.fiscalYearStart||'').slice(0,10)+'"></div>';
    document.getElementById('erpModalSaveBtn').onclick = function() {
      _erpPost('/erp/companies', {
        id: document.getElementById('coId').value || undefined,
        name: document.getElementById('coName').value,
        legalName: document.getElementById('coLegal').value,
        taxNumber: document.getElementById('coTax').value,
        crNumber: document.getElementById('coCR').value,
        country: document.getElementById('coCountry').value,
        city: document.getElementById('coCity').value,
        baseCurrency: document.getElementById('coCur').value,
        fiscalYearStart: document.getElementById('coFY').value || null
      }, function(r){ if (r.success){showToast('تم الحفظ');erpCloseModal();erpLoadCompanies();}else showToast(r.error||'فشل',true); });
    };
    document.getElementById('erpModal').classList.remove('hidden');
  };
  if (id) _erpGet('/erp/companies', function(list){ doRender((list||[]).find(function(x){return x.id===id;})); });
  else doRender();
}
...
function erpOpenWasteModal() {
  Promise.all([
    ...
    var reasons = [['expired','منتهي الصلاحية'],['damaged','تالف'],['spill','انسكاب'],['prep_loss','خسارة تحضير'],['customer_return','إرجاع عميل'],['other','أخرى']];
    document.getElementById('erpModalTitle').textContent = 'تسجيل هدر جديد';
    document.getElementById('erpModalBody').innerHTML =
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">'+
      '<div class="form-row"><label>التاريخ</label><input type="date" class="form-control" id="wnDate" value="'+new Date().toISOString().slice(0,10)+'"></div>'+
      '<div class="form-row"><label>السبب</label><select class="form-control" id="wnReason">'+reasons.map(function(r){return '<option value="'+r[0]+'">'+r[1]+'</option>';}).join('')+'</select></div></div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">'+
      '<div class="form-row"><label>الفرع</label><select class="form-control" id="wnBranch"><option value="">—</option>'+branches.map(function(b){return '<option value="'+b.id+'">'+b.name+'</option>';}).join('')+'</select></div>'+
      '<div class="form-row"><label>البراند</label><select class="form-control" id="wnBrand"><option value="">—</option>'+brands.map(function(b){return '<option value="'+b.id+'">'+b.name+'</option>';}).join('')+'</select></div></div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">'+
      '<div class="form-row"><label>المستودع *</label><select class="form-control" id="wnWh"><option value="">—</option>'+whs.map(function(w){return '<option value="'+w.id+'">'+w.name+'</option>';}).join('')+'</select></div>'+
      '<div class="form-row"><label>مركز التكلفة</label><select class="form-control" id="wnCC"><option value="">—</option>'+ccs.map(function(c){return '<option value="'+c.id+'">'+(c.name||c.code)+'</option>';}).join('')+'</select></div></div>'+
      '<div class="form-row"><label>ملاحظات</label><textarea class="form-control" id="wnNotes" rows="2"></textarea></div>'+
      '<h4 style="margin:14px 0 6px;"><i class="fas fa-boxes"></i> الأصناف المهدورة</h4>'+
      '<div id="wnItems"></div>'+
      '<button class="btn btn-light btn-sm" onclick="erpWasteAddRow()"><i class="fas fa-plus"></i> صنف جديد</button>';
    window._wnItemOptions = '<option value="">—</option>' + items.map(function(i){return '<option value="'+i.id+'" data-cost="'+(i.avg_cost||0)+'" data-unit="'+(i.unit||'PCS')+'">'+(i.name||'')+'</option>';}).join('');
    erpWasteAddRow(); erpWasteAddRow();
    document.getElementById('erpModalSaveBtn').onclick = function() {
      var items = [];
      document.querySelectorAll('.wn-item-row').forEach(function(row){
        var s = row.querySelector('.wn-item-sel');
        var q = row.querySelector('.wn-item-qty');
        var c = row.querySelector('.wn-item-cost');
        if (s.value && Number(q.value)>0) items.push({ itemId: s.value, quantity: Number(q.value), unit: s.options[s.selectedIndex].getAttribute('data-unit'), unitCost: Number(c.value)||0 });
      });
      if (!items.length) return showToast('أضف صنفاً واحداً على الأقل', true);
      _erpPost('/erp/waste-entries', {
        wasteDate: document.getElementById('wnDate').value,
        reason: document.getElementById('wnReason').value,
        branchId: document.getElementById('wnBranch').value || null,
        brandId: document.getElementById('wnBrand').value || null,
        warehouseId: document.getElementById('wnWh').value,
        costCenterId: document.getElementById('wnCC').value || null,
        notes: document.getElementById('wnNotes').value,
        createdBy: currentUser,
        items: items
      }, function(r){ if(r.success){showToast('تم — إجمالي: '+_erpFmt(r.totalCost));erpCloseModal();erpLoadWasteEntries();}else showToast(r.error||'فشل',true); });
    };
    document.getElementById('erpModal').classList.remove('hidden');
  });
}
function erpWasteAddRow() {
  var c = document.getElementById('wnItems');
  var div = document.createElement('div');
  div.className = 'wn-item-row';
  div.style.cssText = 'display:grid;grid-template-columns:3fr 1fr 1fr auto;gap:6px;margin-bottom:6px;';
  div.innerHTML = '<select class="form-control wn-item-sel" onchange="this.parentNode.querySelector(\'.wn-item-cost\').value=this.options[this.selectedIndex].getAttribute(\'data-cost\')||0">'+window._wnItemOptions+'</select>'+
    '<input type="number" step="0.001" class="form-control wn-item-qty" placeholder="الكمية">'+
    '<input type="number" step="0.0001" class="form-control wn-item-cost" placeholder="تكلفة الوحدة">'+
    '<button class="btn btn-sm btn-danger" onclick="this.parentNode.remove()"><i class="fas fa-times"></i></button>';
  c.appendChild(div);
}

// ─── Royalty Runs ───
function erpLoadRoyaltyRuns() {
  document.getElementById('rrunsBody').innerHTML = '<tr><td colspan="9" class="empty-msg"><i class="fas fa-spinner fa-spin"></i></td></tr>';
  _erpGet('/erp/royalty-runs', function(list) {
    if (!Array.isArray(list) || !list.length) { document.getElementById('rrunsBody').innerHTML = '<tr><td colspan="9" class="empty-msg">لا توجد احتسابات</td></tr>'; return; }
    var statMap = {draft:'مسودة',approved:'معتمد',invoiced:'مفوتر',paid:'مدفوع',cancelled:'ملغى'};
    var statClr = {draft:'yellow',approved:'blue',invoiced:'purple',paid:'green',cancelled:'red'};
    document.getElementById('rrunsBody').innerHTML = list.map(function(r){
      var btns = '';
      if (r.status === 'draft') btns += '<button class="btn btn-sm btn-success" onclick="erpRoyaltyApprove(\''+r.id+'\')"><i class="fas fa-check"></i> اعتماد</button> ';
      if (r.status === 'approved' || r.status === 'invoiced') btns += '<button class="btn btn-sm" style="background:#dcfce7;color:#166534;" onclick="erpRoyaltyMarkPaid(\''+r.id+'\')"><i class="fas fa-dollar-sign"></i> دفع</button> ';
      if (r.status === 'draft') btns += '<button class="btn btn-sm btn-danger" onclick="erpRoyaltyDelete(\''+r.id+'\')"><i class="fas fa-trash"></i></button>';
      return '<tr><td>'+(r.runDate||'').slice(0,10)+'</td><td>'+(r.brandName||'')+'</td><td>'+(r.periodStart||'').slice(0,10)+' → '+(r.periodEnd||'').slice(0,10)+'</td>'+
        '<td>'+_erpFmt(r.grossSales)+'</td><td>'+_erpFmt(r.netSales)+'</td><td>'+(r.royaltyType||'')+'</td>'+
        '<td style="font-weight:700;">'+_erpFmt(r.royaltyAmount)+'</td>'+
        '<td><span class="badge badge-'+statClr[r.status]+'">'+(statMap[r.status]||r.status)+'</span></td>'+
        '<td>'+btns+'</td></tr>';
    }).join('');
  });
}
function erpOpenRoyaltyComputeModal() {
  _erpGet('/erp/brands', function(brands) {
    document.getElementById('erpModalTitle').textContent = 'احتساب فرانشايز جديد';
    document.getElementById('erpModalBody').innerHTML =
      '<div class="form-row"><label>البراند *</label><select class="form-control" id="rrnBrand">'+(brands||[]).map(function(b){return '<option value="'+b.id+'">'+b.name+'</option>';}).join('')+'</select></div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;"><div class="form-row"><label>بداية الفترة *</label><input type="date" class="form-control" id="rrnStart"></div><div class="form-row"><label>نهاية الفترة *</label><input type="date" class="form-control" id="rrnEnd"></div></div>';
    document.getElementById('erpModalSaveBtn').onclick = function() {
      var d = { brandId: document.getElementById('rrnBrand').value, periodStart: document.getElementById('rrnStart').value, periodEnd: document.getElementById('rrnEnd').value };
      if (!d.brandId || !d.periodStart || !d.periodEnd) return showToast('البيانات مطلوبة', true);
      _erpPost('/erp/royalty-runs/compute', d, function(r){
        if (r.success) { showToast('تم — المستحق: '+_erpFmt(r.royaltyAmount)); erpCloseModal(); erpLoadRoyaltyRuns(); }
...
function erpRoyaltyApprove(id) {
  if (!confirm('اعتماد الاحتساب وإنشاء قيد محاسبي؟')) return;
  _erpPost('/erp/royalty-runs/'+id+'/approve', { username: currentUser }, function(r){
    if (r.success) { showToast('اعتُمد — قيد #'+(r.journalNumber||'')); erpLoadRoyaltyRuns(); }
    else showToast(r.error||'فشل', true);
  });
}
function erpRoyaltyMarkPaid(id) {
  if (!confirm('تأكيد الدفع؟')) return;
  _erpPost('/erp/royalty-runs/'+id+'/mark-paid', {}, function(r){ if(r.success){showToast('تم');erpLoadRoyaltyRuns();}else showToast(r.error||'فشل',true); });
}
function erpRoyaltyDelete(id) {
  if (!confirm('حذف المسودة؟')) return;
  _erpDelete('/erp/royalty-runs/'+id, function(r){ if(r.success){showToast('حُذفت');erpLoadRoyaltyRuns();}else showToast(r.error||'فشل',true); });
}

// ─── POS Terminals ───
function erpLoadPosTerminals() {
  document.getElementById('posTermBody').innerHTML = '<tr><td colspan="6" class="empty-msg"><i class="fas fa-spinner fa-spin"></i></td></tr>';
  _erpGet('/erp/pos-terminals', function(list) {
    if (!Array.isArray(list) || !list.length) { document.getElementById('posTermBody').innerHTML = '<tr><td colspan="6" class="empty-msg">لا توجد أجهزة</td></tr>'; return; }
    document.getElementById('posTermBody').innerHTML = list.map(function(t){
      return '<tr><td style="font-weight:700;">'+(t.name||'')+'</td><td><code>'+(t.code||'')+'</code></td><td>'+(t.branchName||'')+'</td><td>'+(t.deviceId||'—')+'</td><td>'+(t.lastSyncAt||'—')+'</td>'+
        '<td><button class="btn btn-sm btn-primary" onclick="erpOpenPosTerminalModal(\''+t.id+'\')"><i class="fas fa-edit"></i></button> '+
        '<button class="btn btn-sm btn-danger" onclick="erpDeletePosTerminal(\''+t.id+'\')"><i class="fas fa-trash"></i></button></td></tr>';
    }).join('');
  });
}
function erpOpenPosTerminalModal(id) {
  _erpGet('/erp/branches-full', function(branches){
    var d = {};
    var render = function(){
      var bOpts = (branches||[]).map(function(b){return '<option value="'+b.id+'"'+(d.branchId===b.id?' selected':'')+'>'+b.name+'</option>';}).join('');
      document.getElementById('erpModalTitle').textContent = d.id ? 'تعديل جهاز' : 'جهاز POS جديد';
      document.getElementById('erpModalBody').innerHTML =
        '<input type="hidden" id="ptId" value="'+(d.id||'')+'">'+
        '<div class="form-row"><label>الاسم *</label><input class="form-control" id="ptName" value="'+(d.name||'')+'"></div>'+
        '<div class="form-row"><label>الرمز</label><input class="form-control" id="ptCode" value="'+(d.code||'')+'"></div>'+
        '<div class="form-row"><label>الفرع *</label><select class="form-control" id="ptBranch">'+bOpts+'</select></div>'+
        '<div class="form-row"><label>معرّف الجهاز</label><input class="form-control" id="ptDev" value="'+(d.deviceId||'')+'"></div>';
      document.getElementById('erpModalSaveBtn').onclick = function() {
        _erpPost('/erp/pos-terminals', {
          id: document.getElementById('ptId').value || undefined,
          name: document.getElementById('ptName').value,
          code: document.getElementById('ptCode').value,
          branchId: document.getElementById('ptBranch').value,
          deviceId: document.getElementById('ptDev').value
        }, function(r){ if(r.success){showToast('تم');erpCloseModal();erpLoadPosTerminals();}else showToast(r.error||'فشل',true); });
      };
      document.getElementById('erpModal').classList.remove('hidden');
    };
    if (id) _erpGet('/erp/pos-terminals', function(list){ d = (list||[]).find(function(x){return x.id===id;}) || {}; render(); });
    else render();
  });
}
function erpDeletePosTerminal(id) {
  if (!confirm('تعطيل الجهاز؟')) return;
  _erpDelete('/erp/pos-terminals/'+id, function(r){ if(r.success){showToast('تم');erpLoadPosTerminals();}else showToast(r.error||'فشل',true); });
}

// ─── Item Categories ───
function erpLoadItemCategories() {
  document.getElementById('catBody').innerHTML = '<tr><td colspan="6" class="empty-msg"><i class="fas fa-spinner fa-spin"></i></td></tr>';
  _erpGet('/erp/item-categories', function(list) {
    if (!Array.isArray(list) || !list.length) { document.getElementById('catBody').innerHTML = '<tr><td colspan="6" class="empty-msg">لا توجد تصنيفات</td></tr>'; return; }
    document.getElementById('catBody').innerHTML = list.map(function(c){
      return '<tr><td style="font-weight:700;">'+(c.name||'')+'</td><td><code>'+(c.code||'')+'</code></td><td>'+(c.parentName||'—')+'</td><td>'+(c.brandId||'—')+'</td><td><span class="badge badge-blue">'+c.itemCount+'</span></td>'+
        '<td><button class="btn btn-sm btn-primary" onclick="erpOpenCategoryModal(\''+c.id+'\')"><i class="fas fa-edit"></i></button> '+
        '<button class="btn btn-sm btn-danger" onclick="erpDeleteCategory(\''+c.id+'\')"><i class="fas fa-trash"></i></button></td></tr>';
    }).join('');
  });
}
function erpOpenCategoryModal(id) {
  Promise.all([
    new Promise(function(r){_erpGet('/erp/item-categories',r);}),
    new Promise(function(r){_erpGet('/erp/brands',r);})
  ]).then(function(res){
    var cats = res[0]||[], brands = res[1]||[];
    var d = {};
    var render = function() {
      var parentOpts = '<option value="">— جذر —</option>' + cats.filter(function(c){return c.id!==d.id;}).map(function(c){return '<option value="'+c.id+'"'+(d.parentId===c.id?' selected':'')+'>'+c.name+'</option>';}).join('');
      var brOpts = '<option value="">— عام —</option>' + brands.map(function(b){return '<option value="'+b.id+'"'+(d.brandId===b.id?' selected':'')+'>'+b.name+'</option>';}).join('');
      document.getElementById('erpModalTitle').textContent = d.id ? 'تعديل تصنيف' : 'تصنيف جديد';
      document.getElementById('erpModalBody').innerHTML =
        '<input type="hidden" id="catId" value="'+(d.id||'')+'">'+
        '<div class="form-row"><label>الاسم *</label><input class="form-control" id="catName" value="'+(d.name||'').replace(/"/g,'&quot;')+'"></div>'+
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;"><div class="form-row"><label>الرمز</label><input class="form-control" id="catCode" value="'+(d.code||'')+'"></div><div class="form-row"><label>التصنيف الأب</label><select class="form-control" id="catParent">'+parentOpts+'</select></div></div>'+
        '<div class="form-row"><label>البراند</label><select class="form-control" id="catBrand">'+brOpts+'</select></div>';
      document.getElementById('erpModalSaveBtn').onclick = function() {
        _erpPost('/erp/item-categories', {
          ...
          brandId: document.getElementById('catBrand').value || null
        }, function(r){ if(r.success){showToast('تم');erpCloseModal();erpLoadItemCategories();}else showToast(r.error||'فشل',true); });
      };
      document.getElementById('erpModal').classList.remove('hidden');
    };
    if (id) { d = (cats||[]).find(function(x){return x.id===id;}) || {}; }
    render();
  });
}
function erpDeleteCategory(id) {
  if (!confirm('حذف التصنيف؟')) return;
  _erpDelete('/erp/item-categories/'+id, function(r){ if(r.success){showToast('تم');erpLoadItemCategories();}else showToast(r.error||'فشل',true); });
}
...
function whhLoad() {
  ...
  _erpGet('/erp/warehouses/hierarchy', function(rows) {
    ...
    var byBrand = {};
    rows.forEach(function(r) {
      var k = r.brand_id || '__none__';
      if (!byBrand[k]) byBrand[k] = { mains: [], others: [] };
    ...
    Object.keys(byBrand).forEach(function(brandKey) {
      ...
      group.mains.forEach(function(m) {
        var children = rows.filter(function(r){return r.parent_warehouse_id === m.id;});
        html += _whhCardHTML(m, children, rows);
      });
      group.others.forEach(function(m) {
...
function whhSetMain(id) {
  if (!confirm('تعيين هذا المستودع كرئيسي للبرند؟ (سيتم إزالة العلم من المستودعات الأخرى)')) return;
  _erpPost('/erp/warehouses/'+id+'/set-main', {}, function(r){
    if (r.success) { showToast('تم التعيين'); whhLoad(); }
    else showToast(r.error||'فشل', true);
  });
}
...
function siAddItem() {
  ...
  document.getElementById('siItemQty').value = '';
  _siRenderItems();
}
function _siRenderItems() {
  var tb = document.getElementById('siItemsRows');
  if (!tb) return;
  if (!window._siItems || !window._siItems.length) { tb.innerHTML = '<tr><td colspan="3" style="color:#94a3b8;text-align:center;">لم تُضف أصناف</td></tr>'; return; }
  tb.innerHTML = window._siItems.map(function(it, i){
...
function siIssue(id) {
  if (!confirm('صرف الكميات من المستودع الرئيسي الآن؟ (سيتم ترحيل GL تلقائياً)')) return;
  _erpPost('/erp/stock-issues/'+id+'/issue', { issuedBy: currentUser }, function(r){
    if (r.success) { showToast('تم الصرف + ترحيل قيد GL'); siLoad(); } else showToast(r.error||'فشل', true);
  });
}
function siReceive(id) {
  if (!confirm('تأكيد استلام الكميات في المستودع الفرعي؟')) return;
  _erpPost('/erp/stock-issues/'+id+'/receive', { receivedBy: currentUser }, function(r){
    if (r.success) { showToast('تم الاستلام'); siLoad(); } else showToast(r.error||'فشل', true);
  });
}
function siCancel(id) {
  if (!confirm('إلغاء هذا الإذن؟')) return;
  _erpPost('/erp/stock-issues/'+id+'/cancel', {}, function(r){
    if (r.success) { showToast('تم الإلغاء'); siLoad(); } else showToast(r.error||'فشل', true);
  });
}
...
function prdLoadInit() {
  _erpGet('/erp/warehouses-list', function(whs) {
    var opts = '<option value="">كل المستودعات</option>' + (whs||[]).map(function(w){return '<option value="'+w.id+'">'+(w.name||'')+'</option>';}).join('');
    var f = document.getElementById('prdFWh'); if (f) f.innerHTML = opts;
    _erpGet('/erp/brands', function(brs){
      var fb = document.getElementById('prdFBrand');
      if (fb) fb.innerHTML = '<option value="">كل البراندات</option>' + (brs||[]).map(function(b){return '<option value="'+b.id+'">'+_woEscapeHtml(b.name||'')+'</option>';}).join('');
      prdLoad();
    });
  });
}
...
function prdLoad() {
  ...
  if (!body) return;
  body.innerHTML = _woLoadingRow(10) + _woLoadingRow(10);
  var q = '';
  var st = document.getElementById('prdFStatus').value;
  var wh = document.getElementById('prdFWh').value;
  var br = document.getElementById('prdFBrand') ? document.getElementById('prdFBrand').value : '';
  var from = document.getElementById('prdFFrom').value;
  var to = document.getElementById('prdFTo').value;
  if (st) q += '&status=' + st;
  ...
  _erpGet('/erp/production-orders?1=1' + q, function(rows){
    ...
    if (!rows.length) {
      body.innerHTML = '<tr><td colspan="10">' + _woEmpty('fa-industry', 'لا توجد أوامر إنتاج', 'أنشئ أول أمر إنتاج من وصفة BOM لبدء احتساب التكاليف الفعلية.',
    ...
    body.innerHTML = rows.map(function(r){
      ...
      var brandChip = r.brand_name
        ? '<span class="wo-chip purple"><i class="fas fa-store"></i> '+_woEscapeHtml(r.brand_name)+'</span>'
        : '<span class="wo-text-subtle wo-text-caption">—</span>';
      return '<tr>' +
...
var _prdDraftKey = 'prd_draft_v2';
var _prdState = { brandId:'', bomId:'', qty:'', whRaw:'', whOut:'', date:new Date().toISOString().slice(0,10), notes:'' };
var _prdAllBoms = [];
var _prdAllWhs = [];
var _prdAllBrands = [];
...
function _prdLoadDraft() {
  try {
    var raw = localStorage.getItem(_prdDraftKey);
    if (raw) {
...
function _prdClearDraft() {
  try { localStorage.removeItem(_prdDraftKey); } catch(e) {}
  _prdState = { brandId:'', bomId:'', qty:'', whRaw:'', whOut:'', date:new Date().toISOString().slice(0,10), notes:'' };
}

function prdOpenNewModal() {
  _prdLoadDraft();
  // Parallel load: brands, BOMs, warehouses
  var pending = 3;
  var data = { brands:[], boms:[], whs:[] };
  var done = function() { if (--pending === 0) _prdRenderModal(data); };
  _erpGet('/erp/brands', function(r){ data.brands = r || []; done(); });
  _erpGet('/erp/bom',    function(r){ data.boms   = r || []; done(); });
  _erpGet('/erp/warehouses-list', function(r){ data.whs = r || []; done(); });
}

function _prdRenderModal(data) {
  _prdAllBrands = data.brands; _prdAllBoms = data.boms; _prdAllWhs = data.whs;

  var brandOpts = '<option value="">— كل البراندات —</option>' +
    _prdAllBrands.map(function(b){
      var sel = (_prdState.brandId === b.id) ? ' selected' : '';
      return '<option value="'+b.id+'"'+sel+'>'+_woEscapeHtml(b.name||'')+'</option>';
    }).join('');

  var body =
    '<div class="wo-section" style="padding:0;box-shadow:none;background:transparent;">' +
      // Draft indicator
      '<div id="prdDraftBadge" class="wo-chip info" style="margin-bottom:12px;display:none;">' +
        '<i class="fas fa-floppy-disk"></i> مسودة محفوظة — سيتم استرجاع القيم تلقائياً' +
      '</div>' +

      // Brand selector (Foodics-style: choose brand first → filters everything else)
      '<div class="wo-label-stack" style="margin-bottom:14px;">' +
        '<label class="wo-field-label" for="prdBrand">' +
          '<i class="fas fa-store" style="color:var(--wo-purple);"></i> البراند' +
          ' <span class="wo-text-caption" style="color:var(--wo-on-surface-subtle);">— يحدد المواد والمستودعات المتاحة</span>' +
        '</label>' +
        '<select class="wo-select" id="prdBrand">'+brandOpts+'</select>' +
      '</div>' +

      // BOM picker (searchable, brand-filtered)
      '<div class="wo-label-stack" style="margin-bottom:14px;">' +
        '<label class="wo-field-label">' +
          '<i class="fas fa-book-open" style="color:var(--wo-info-fg);"></i> وصفة الإنتاج (BOM) *' +
        '</label>' +
        '<div id="prdBomPickerHost"></div>' +
        '<div id="prdBomSelected" class="wo-chip success" style="display:none;margin-top:8px;">' +
          '<i class="fas fa-check"></i> <span id="prdBomSelectedText"></span>' +
          ' <button type="button" onclick="_prdClearBom()" class="wo-icon-btn danger" style="width:22px;height:22px;margin-inline-start:6px;" aria-label="مسح"><i class="fas fa-xmark"></i></button>' +
        '</div>' +
      ...
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">' +
        '<div class="wo-label-stack">' +
          '<label class="wo-field-label" for="prdQty"><i class="fas fa-boxes-stacked"></i> الكمية المراد إنتاجها *</label>' +
          '<input type="number" class="wo-input" id="prdQty" step="0.001" min="0" value="'+_woEscapeHtml(_prdState.qty||'')+'" placeholder="مثلاً: 50">' +
          '<span id="prdYieldHint" class="wo-text-caption" style="color:var(--wo-on-surface-subtle);min-height:14px;"></span>' +
        '</div>' +
        '<div class="wo-label-stack">' +
          '<label class="wo-field-label" for="prdDate"><i class="fas fa-calendar-day"></i> تاريخ الإنتاج المخطط</label>' +
          '<input type="date" class="wo-input" id="prdDate" value="'+_woEscapeHtml(_prdState.date||new Date().toISOString().slice(0,10))+'">' +
        '</div>' +
      '</div>' +

      // Warehouses row (brand-filtered)
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">' +
        '<div class="wo-label-stack">' +
          '<label class="wo-field-label" for="prdWhRaw"><i class="fas fa-warehouse" style="color:var(--wo-warning-fg);"></i> مستودع المواد الخام *</label>' +
          '<select class="wo-select" id="prdWhRaw"><option value="">— اختر المستودع —</option></select>' +
          '<span class="wo-text-caption" style="color:var(--wo-on-surface-subtle);">تُصرف منه المكونات</span>' +
        '</div>' +
        '<div class="wo-label-stack">' +
          '<label class="wo-field-label" for="prdWhOut"><i class="fas fa-check-double" style="color:var(--wo-success-fg);"></i> مستودع المنتج النهائي</label>' +
          '<select class="wo-select" id="prdWhOut"><option value="">(نفس مستودع المواد الخام)</option></select>' +
          '<span class="wo-text-caption" style="color:var(--wo-on-surface-subtle);">يودَع فيه المنتج بعد الإنتاج</span>' +
        '</div>' +
      '</div>' +

      // Notes
      '<div class="wo-label-stack">' +
        '<label class="wo-field-label" for="prdNotes"><i class="fas fa-note-sticky"></i> ملاحظات (اختياري)</label>' +
        '<textarea class="wo-textarea" id="prdNotes" rows="2" placeholder="ملاحظات على أمر الإنتاج...">'+_woEscapeHtml(_prdState.notes||'')+'</textarea>' +
      '</div>' +
    '</div>';

  var footer =
    '<button class="wo-btn wo-btn-secondary" onclick="_prdDiscardDraft()"><i class="fas fa-trash"></i><span>تفريغ المسودة</span></button>' +
    '<button class="wo-btn wo-btn-secondary" onclick="window._prdModalApi && window._prdModalApi.close()">إلغاء</button>' +
    '<button class="wo-btn wo-btn-primary" onclick="prdSubmitNew()"><i class="fas fa-industry"></i><span>إنشاء أمر الإنتاج</span></button>';

  window._prdModalApi = WoModal.open({
    id: 'prdNewModal',
    icon: 'fa-industry', iconColor: 'warning',
    title: 'أمر إنتاج جديد',
    subtitle: 'اختر البراند لتصفية الوصفات والمستودعات المتاحة',
    body: body, footer: footer, size: 'lg',
    onClose: function() { _prdSaveDraft(); }
  });

  // Apply draft restoration
  var hasDraft = _prdState.bomId || _prdState.whRaw || _prdState.qty;
  if (hasDraft) document.getElementById('prdDraftBadge').style.display = 'inline-flex';

  // Wire brand change → re-populate BOMs + warehouses
  document.getElementById('prdBrand').addEventListener('change', function(){
    _prdState.brandId = this.value; _prdSaveDraft();
    _prdMountBomPicker();
    _prdPopulateWarehouses();
    _prdClearBom();
  });

  // Quantity input → live yield hint + persist
  document.getElementById('prdQty').addEventListener('input', function(){
    _prdState.qty = this.value; _prdSaveDraft(); _prdUpdateYieldHint();
  });
  document.getElementById('prdDate').addEventListener('change', function(){ _prdState.date = this.value; _prdSaveDraft(); });
  document.getElementById('prdNotes').addEventListener('input', function(){ _prdState.notes = this.value; _prdSaveDraft(); });

  // Initial population
  _prdMountBomPicker();
  _prdPopulateWarehouses();
  if (_prdState.bomId) {
    var pre = _prdAllBoms.find(function(b){ return b.id === _prdState.bomId; });
    if (pre) _prdApplyBomSelection(pre);
  }
}

function _prdFilteredBoms() {
  if (!_prdState.brandId) return _prdAllBoms;
  return _prdAllBoms.filter(function(b){ return b.brandId === _prdState.brandId; });
}

function _prdMountBomPicker() {
  var host = document.getElementById('prdBomPickerHost');
  if (!host || !window.WoItemPicker) return;
  var bomsAsItems = _prdFilteredBoms().map(function(b){
    return {
      id: b.id,
      name: b.productName || b.productId || '',
      category: 'BOM v' + (b.version||1),
      unit: b.yieldUnit || 'PCS',
      brandName: b.brandName || '',
      stock: (b.yieldQuantity||1) + ' لكل دفعة',
      cost: null,
      _bom: b
    };
  });
  WoItemPicker.mount(host, {
    items: bomsAsItems,
    placeholder: bomsAsItems.length
      ? 'ابحث عن وصفة بالاسم أو الكود... ('+bomsAsItems.length+' وصفة متاحة)'
      : (_prdState.brandId ? 'لا توجد وصفات لهذا البراند — اضف من قسم الوصفات' : 'لا توجد وصفات مُعرّفة'),
    onSelect: function(item) { _prdApplyBomSelection(item._bom); }
  });
}
...
function _prdApplyBomSelection(bom) {
  ...
  var selText = document.getElementById('prdBomSelectedText');
  if (sel && selText) {
  ...
  _erpGet('/erp/bom/'+bom.id+'/lines', function(lines){
    ...
    if (!preview) return;
    if (!_prdBomLines.length) { preview.style.display = 'none'; return; }
    var rows = _prdBomLines.map(function(l){
      return '<tr>' +
        '<td>'+_woEscapeHtml(l.itemName||'')+'</td>' +
        '<td class="num">'+l.quantity+' '+_woEscapeHtml(l.unit||'')+'</td>' +
        '<td class="num">'+Number(l.avgCost||0).toFixed(4)+'</td>' +
        '<td class="num">'+Number(l.lineCost||0).toFixed(2)+'</td>' +
      '</tr>';
    }).join('');
    ...
    preview.innerHTML =
      ...
      '<table class="wo-table" style="font-size:12px;margin:0;">' +
        '<thead><tr><th>الصنف</th><th class="num">الكمية</th><th class="num">تكلفة الوحدة</th><th class="num">إجمالي السطر</th></tr></thead>' +
...
function _prdUpdateYieldHint() {
  var hint = document.getElementById('prdYieldHint'); if (!hint) return;
  var qty = Number(_prdState.qty || 0);
  if (!_prdState.bomId || !qty) { hint.textContent = ''; return; }
  var bom = _prdAllBoms.find(function(b){ return b.id === _prdState.bomId; });
  if (!bom) { hint.textContent = ''; return; }
  var batches = qty / (bom.yieldQuantity||1);
  hint.innerHTML = '<i class="fas fa-calculator"></i> أي ' + batches.toFixed(2) + ' دفعة × ' + (bom.yieldQuantity||1) + ' ' + (bom.yieldUnit||'') +
    (_prdBomLines.length ? ' — سيُصرف ' + _prdBomLines.length + ' نوع مواد' : '');
}

function _prdPopulateWarehouses() {
  var filtered = _prdState.brandId
    ? _prdAllWhs.filter(function(w){ return !w.brandId || w.brandId === _prdState.brandId; })
    : _prdAllWhs;
  var raw = document.getElementById('prdWhRaw');
  var out = document.getElementById('prdWhOut');
  if (!raw || !out) return;
  var opts = filtered.map(function(w){
    var brandLbl = w.brandName ? ' · '+w.brandName : '';
    return '<option value="'+w.id+'" data-brand="'+_woEscapeHtml(w.brandId||'')+'">'+_woEscapeHtml(w.name||'')+brandLbl+'</option>';
  }).join('');
  raw.innerHTML = '<option value="">— اختر المستودع —</option>'+opts;
  out.innerHTML = '<option value="">(نفس مستودع المواد الخام)</option>'+opts;
  if (_prdState.whRaw) raw.value = _prdState.whRaw;
  if (_prdState.whOut) out.value = _prdState.whOut;
  raw.addEventListener('change', function(){ _prdState.whRaw = this.value; _prdSaveDraft(); });
  out.addEventListener('change', function(){ _prdState.whOut = this.value; _prdSaveDraft(); });
}

function _prdDiscardDraft() {
  WoModal.confirm({
    title: 'تفريغ المسودة؟',
    message: 'سيتم مسح كل القيم المحفوظة في هذا النموذج.',
    confirmText: 'تفريغ', danger: true
  }).then(function(ok){
    if (!ok) return;
    _prdClearDraft();
...
function prdSubmitNew() {
  // Read from state (persisted) + live DOM for final values
  _prdState.qty   = document.getElementById('prdQty').value;
  _prdState.whRaw = document.getElementById('prdWhRaw').value;
  _prdState.whOut = document.getElementById('prdWhOut').value;
  _prdState.date  = document.getElementById('prdDate').value;
  _prdState.notes = document.getElementById('prdNotes').value;
  _prdSaveDraft();
...
function prdView(id) {
  _erpGet('/erp/production-orders/'+id, function(d){
    if (!d || d.error) return showToast(d?d.error:'فشل', true);
    var consHtml = (d.consumption||[]).map(function(c){
      return '<tr><td>'+(c.item_name||'')+'</td><td>'+(c.qty_planned||0)+'</td><td>'+(c.qty_actual||0)+'</td><td>'+Number(c.unit_cost||0).toFixed(4)+'</td><td>'+Number(c.total_cost||0).toFixed(2)+'</td></tr>';
    }).join('');
    var outHtml = (d.output||[]).map(function(o){
      return '<tr><td>'+(o.item_name||'')+'</td><td>'+(o.qty||0)+'</td><td>'+Number(o.unit_cost||0).toFixed(4)+'</td><td>'+Number(o.total_cost||0).toFixed(2)+'</td><td>'+(o.batch_number||'—')+'</td><td>'+(o.expiry_date||'—')+'</td></tr>';
    }).join('');
    document.getElementById('erpModalTitle').textContent = 'أمر إنتاج — '+d.order_number;
    document.getElementById('erpModalBody').innerHTML =
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:12px;">' +
        '<div><small>المنتج</small><div><b>'+(d.product_name||'')+'</b></div></div>' +
        '<div><small>الحالة</small><div>'+(d.status||'')+'</div></div>' +
        '<div><small>الكمية</small><div>'+(d.qty_planned||0)+' (أُنتج: '+(d.qty_produced||0)+')</div></div>' +
        '<div><small>الإجمالي</small><div><b>'+Number(d.total_cost||0).toFixed(2)+'</b></div></div>' +
        '<div><small>مواد</small><div>'+Number(d.materials_cost||0).toFixed(2)+'</div></div>' +
        '<div><small>عمالة + غير مباشرة</small><div>'+Number(Number(d.labor_cost||0)+Number(d.overhead_cost||0)).toFixed(2)+'</div></div>' +
      '</div>' +
      '<h4 style="margin:12px 0 6px;font-size:13px;"><i class="fas fa-boxes"></i> استهلاك المواد الخام</h4>' +
      '<table class="erp-table" style="font-size:12px;"><thead><tr><th>الصنف</th><th>مخطط</th><th>فعلي</th><th>تكلفة</th><th>إجمالي</th></tr></thead><tbody>'+(consHtml||'<tr><td colspan="5" style="color:#94a3b8;text-align:center;">—</td></tr>')+'</tbody></table>' +
      '<h4 style="margin:12px 0 6px;font-size:13px;"><i class="fas fa-check-double"></i> المنتج النهائي</h4>' +
      '<table class="erp-table" style="font-size:12px;"><thead><tr><th>الصنف</th><th>الكمية</th><th>تكلفة الوحدة</th><th>إجمالي</th><th>دفعة</th><th>انتهاء</th></tr></thead><tbody>'+(outHtml||'<tr><td colspan="6" style="color:#94a3b8;text-align:center;">لم يُكتمل بعد</td></tr>')+'</tbody></table>';
    document.getElementById('erpModal').classList.remove('hidden');
  });
}
function prdRelease(id) {
  var labor = prompt('تكلفة العمالة (اختياري):', '0');
  if (labor === null) return;
  var overhead = prompt('التكاليف غير المباشرة (اختياري):', '0');
  if (overhead === null) return;
  _erpPost('/erp/production-orders/'+id+'/release', {
    releasedBy: currentUser,
    laborCost: Number(labor)||0,
    overheadCost: Number(overhead)||0
  }, function(r){
    if (r.success) { showToast('تم الإطلاق — تكلفة المواد: '+Number(r.materialsCost).toFixed(2)); prdLoad(); }
    else showToast(r.error||'فشل', true);
  });
}
function prdComplete(id) {
  var qty = prompt('الكمية المُنتجة فعلياً:');
  if (!qty || Number(qty) <= 0) return showToast('كمية غير صحيحة', true);
  var scrap = prompt('كمية الهدر (اختياري):', '0');
  if (scrap === null) return;
  var batch = prompt('رقم الدفعة (اختياري):', '');
  var expiry = prompt('تاريخ الانتهاء (YYYY-MM-DD، اختياري):', '');
  _erpPost('/erp/production-orders/'+id+'/complete', {
    ...
    if (r.success) { showToast('تم الإنتاج — تكلفة الوحدة: '+Number(r.unitCost).toFixed(4)); prdLoad(); }
    else showToast(r.error||'فشل', true);
  });
}
...
function _mhRender() {
  ...
  grid.innerHTML = filtered.map(function(b) {
    ...
    var menuCount = b.menuCount || 0;
    var emptyAlert = menuCount === 0
      ? '<div style="background:#fef3c7;color:#92400e;padding:6px 10px;border-radius:8px;font-size:11px;font-weight:700;margin-top:6px;display:flex;align-items:center;gap:5px;"><i class="fas fa-exclamation-triangle"></i> لا توجد منتجات بعد</div>'
      : '';
    return '<div class="mh-brand-card' + (b.isActive ? '' : ' inactive') + '" onclick="erpOpenBrandMenu(\'' + b.id + '\',\'' + (b.name||'').replace(/[\\\\\']/g,'') + '\')">' +
      ...
      '<div class="mh-brand-stats">' +
        '<div class="mh-stat"><i class="fas fa-utensils"></i> ' + menuCount + ' منتج</div>' +
        '<div class="mh-stat"><i class="fas fa-code-branch"></i> ' + (b.branchCount||0) + ' فرع</div>' +
      '</div>' +
      '<div class="mh-brand-cta">' +
        '<span><i class="fas fa-arrow-left"></i> ' + (menuCount === 0 ? 'ابدأ بإضافة منتجات' : 'فتح المنيو') + '</span>' +
...
function _sfRender() {
  ...
  if (metrics) {
    ...
    var lowStock = rows.filter(function(m){ var s = Number(m.stock||0), min = Number(m.minStock||0); return min > 0 && s <= min; }).length;
    var outOfStock = rows.filter(function(m){ return Number(m.stock||0) <= 0; }).length;
    var withBom = rows.filter(function(m){ return m.recipeCount > 0 || m.hasBom; }).length;
    metrics.innerHTML =
  ...
  tbody.innerHTML = filtered.map(function(m) {
    ...
    var min = Number(m.minStock || 0);
    var stockClr = stock <= 0 ? '#ef4444' : (min > 0 && stock <= min ? '#f59e0b' : '#16a34a');
    var stockIcon = stock <= 0 ? 'fa-circle-xmark' : (min > 0 && stock <= min ? 'fa-triangle-exclamation' : 'fa-check-circle');
    var stockLabel = stock <= 0 ? 'نفذ' : (min > 0 && stock <= min ? 'منخفض' : 'متوفر');
    return '<tr>' +
      '<td><div style="font-weight:800;">'+ _v3EscapeHtml(m.name) +'</div>' +
        (m.consumerCount > 0 ? '<div style="font-size:10px;color:#3b82f6;margin-top:2px;"><i class="fas fa-link"></i> يستهلكه ' + m.consumerCount + ' منتج تام</div>' : '') +
      '</td>' +
      '<td>'+ _v3EscapeHtml(m.brandName||'—') +'</td>' +
      '<td><span class="wo-chip">'+ _v3EscapeHtml(m.productionUnit||'pcs') +'</span></td>' +
      '<td>' +
        '<div style="display:flex;align-items:center;gap:6px;">' +
          '<i class="fas '+stockIcon+'" style="color:'+stockClr+';font-size:13px;"></i>' +
          '<div>' +
            '<div style="font-weight:900;color:'+stockClr+';font-size:14px;">' + _v3Fmt(stock) + '</div>' +
            '<div style="font-size:10px;color:'+stockClr+';font-weight:700;">' + stockLabel + (min > 0 ? ' (حد أدنى: ' + min + ')' : '') + '</div>' +
          '</div>' +
        '</div>' +
      '</td>' +
      '<td class="num">' + (m.recipeCount != null ? '<span style="background:#dbeafe;color:#1e40af;padding:2px 8px;border-radius:6px;font-size:11px;font-weight:700;">' + m.recipeCount + ' مكوّن</span>' : '<span style="color:#94a3b8;">— </span>') + '</td>' +
      ...
      '<td>' +
        ...
        '<button class="wo-btn wo-btn-sm wo-btn-warning" onclick="erpQuickProduceModal(\''+ m.id +'\')" title="إنتاج سريع"><i class="fas fa-industry"></i></button> ' +
        '<button class="wo-btn wo-btn-sm wo-btn-danger" onclick="erpDeleteBrandMenuItem(\''+ m.id +'\')" title="حذف"><i class="fas fa-trash"></i></button>' +
      '</td>' +
    '</tr>';
  }).join('');
...
// V3: Quick produce — opens production order pre-filled from this semi-finished
window.erpQuickProduceModal = function(semiId) {
  var sf = (window._sfItemsCache || []).find(function(x){ return String(x.id) === String(semiId); });
  if (!sf) { _v3Toast('غير موجود', true); return; }
  // Cache for the production form to pre-select
  window._poPrefillSemi = sf;
  erpOpenProductionOrderModal();
  setTimeout(function() {
    // Auto-pick this semi in the first row
    var sel = document.querySelector('#poTbl tbody .po-product');
    if (sel) {
      sel.value = sf.id;
      var u = document.querySelector('#poTbl tbody .po-unit');
      if (u) u.value = sf.productionUnit || 'pcs';
    }
    // Auto-pick brand
    var brSel = document.getElementById('poF_brandId');
    if (brSel && sf.brandId) brSel.value = sf.brandId;
  }, 200);
};
...
function _poRender() {
  ...
  if (metrics) {
    var byStatus = { planned:0, released:0, in_progress:0, completed:0, cancelled:0 };
    var totalCost = 0;
    rows.forEach(function(o){
      var st = o.status || 'planned';
      if (byStatus[st]!=null) byStatus[st]++;
      totalCost += Number(o.materialsCost||o.materials_cost||0) + Number(o.laborCost||o.labor_cost||0) + Number(o.overheadCost||o.overhead_cost||0);
    });
    var totalProduced = rows.reduce(function(a,o){return a + Number(o.qtyProduced||o.qty_produced||0);}, 0);
    var totalPlanned = rows.reduce(function(a,o){return a + Number(o.qtyPlanned||o.qty_planned||0);}, 0);
    var efficiency = totalPlanned > 0 ? ((totalProduced / totalPlanned) * 100).toFixed(0) : 0;
    metrics.innerHTML =
  ...
  tbody.innerHTML = rows.map(function(o) {
    var statusMap = {
      planned:['#94a3b8','مخطط','fa-clipboard-list'],
      released:['#f59e0b','مُطلق','fa-play'],
      in_progress:['#06b6d4','قيد التنفيذ','fa-spinner'],
      completed:['#22c55e','مكتمل','fa-check-circle'],
      closed:['#16a34a','مُقفل','fa-lock'],
      cancelled:['#ef4444','ملغي','fa-xmark']
    };
    var st = statusMap[o.status] || ['#64748b', o.status, 'fa-circle'];
    var planned = Number(o.qtyPlanned || o.qty_planned || 0);
    var produced = Number(o.qtyProduced || o.qty_produced || 0);
    var pct = planned > 0 ? Math.min(100, Math.round((produced / planned) * 100)) : 0;
    var pctClr = pct >= 100 ? '#16a34a' : (pct >= 50 ? '#f59e0b' : '#94a3b8');
    var cost = Number(o.materialsCost||o.materials_cost||0) + Number(o.laborCost||o.labor_cost||0) + Number(o.overheadCost||o.overhead_cost||0);

    return '<tr>' +
      '<td><code style="background:#f1f5f9;padding:2px 6px;border-radius:4px;font-size:11px;">'+ _v3EscapeHtml(o.id) +'</code>' +
        (cost > 0 ? '<div style="font-size:10px;color:#be185d;margin-top:2px;"><i class="fas fa-coins"></i> ' + _v3Fmt(cost) + ' ر.س</div>' : '') +
      '</td>' +
      ...
      '<td><div style="font-weight:800;">' + _v3EscapeHtml(o.productName||o.product_name||'—') + '</div>' +
        (o.warehouseName||o.warehouse_id ? '<div style="font-size:10px;color:#64748b;"><i class="fas fa-warehouse"></i> ' + _v3EscapeHtml(o.warehouseName||o.warehouse_id) + '</div>' : '') +
      '</td>' +
      '<td>' +
        '<div style="display:flex;align-items:center;gap:6px;font-size:12px;">' +
          '<b style="color:'+pctClr+';">' + _v3Fmt(produced) + '</b><small style="color:#94a3b8;">/ ' + _v3Fmt(planned) + '</small>' +
        '</div>' +
        '<div style="background:#f1f5f9;border-radius:999px;height:6px;overflow:hidden;margin-top:4px;width:90px;">' +
          '<div style="height:100%;background:' + pctClr + ';width:' + pct + '%;transition:width .3s;"></div>' +
        '</div>' +
        '<div style="font-size:10px;color:'+pctClr+';font-weight:700;margin-top:2px;">' + pct + '%</div>' +
      '</td>' +
      '<td><span class="wo-chip" style="background:'+st[0]+'1a;color:'+st[0]+';font-weight:700;"><i class="fas '+st[2]+'" style="margin-left:4px;"></i>'+ st[1] +'</span></td>' +
      '<td>' +
        (o.status === 'planned' ? '<button class="wo-btn wo-btn-sm wo-btn-warning" onclick="erpReleaseProduction(\''+ o.id +'\')" title="إطلاق + خصم المكونات"><i class="fas fa-play"></i></button> ' : '') +
        ((o.status === 'released' || o.status === 'in_progress') ? '<button class="wo-btn wo-btn-sm wo-btn-success" onclick="erpCompleteProduction(\''+ o.id +'\')" title="إكمال + إضافة المنتج"><i class="fas fa-check"></i></button> ' : '') +
        '<button class="wo-btn wo-btn-sm wo-btn-secondary" onclick="erpViewProductionDetail(\''+ o.id +'\')" title="تفاصيل + مكونات"><i class="fas fa-eye"></i></button> ' +
        '<button class="wo-btn wo-btn-sm wo-btn-secondary" onclick="erpDuplicateProduction(\''+ o.id +'\')" title="نسخ كأمر جديد"><i class="fas fa-copy"></i></button>' +
      '</td>' +
    '</tr>';
  }).join('');
}

window.erpDuplicateProduction = function(id) {
  if (!confirm('هل تريد إنشاء أمر إنتاج جديد بنفس بيانات هذا الأمر (المنتج + الكمية + المستودع)؟')) return;
  callAPI('GET', '/erp/production-orders/' + id, null, function(o) {
    ...
    var payload = {
      ...
      qtyPlanned: o.qtyPlanned || o.qty_planned,
      brandId: o.brandId || o.brand_id,
      warehouseId: o.warehouseId || o.warehouse_id,
      orderDate: new Date().toISOString().slice(0, 10),
      notes: 'مكرر من ' + id
    };
    callAPI('POST', '/erp/production-orders', payload, function(r) {
      if (r && r.success) {
        _v3Toast('تم إنشاء أمر إنتاج جديد');
        erpLoadProductionOrders();
      } else {
        _v3Toast((r && r.error) || 'فشل النسخ', true);
      }
    });
  });
};
...
function erpViewProductionDetail(id) {
  // Load order + consumption + output in parallel
  Promise.all([
    new Promise(function(resolve) { callAPI('GET', '/erp/production-orders/' + id, null, resolve); }),
    new Promise(function(resolve) { callAPI('GET', '/erp/production-orders/' + id + '/consumption', null, function(d){ resolve(Array.isArray(d) ? d : []); }); }),
    new Promise(function(resolve) { callAPI('GET', '/erp/production-orders/' + id + '/output', null, function(d){ resolve(Array.isArray(d) ? d : []); }); })
  ]).then(function(res) {
    var o = res[0], consumption = res[1], output = res[2];
    if (!o || o.error) { _v3Toast('فشل التحميل', true); return; }
    _renderProductionDetailModal(o, consumption, output);
  });
}

function _renderProductionDetailModal(o, consumption, output) {
  var planned = Number(o.qtyPlanned || o.qty_planned || 0);
  var produced = Number(o.qtyProduced || o.qty_produced || 0);
  var scrap = Number(o.qtyScrap || o.qty_scrap || 0);
  var pct = planned > 0 ? Math.min(100, Math.round((produced / planned) * 100)) : 0;
  var pctClr = pct >= 100 ? '#16a34a' : (pct >= 50 ? '#f59e0b' : '#94a3b8');
  var matCost = Number(o.materialsCost || o.materials_cost || 0);
  var labCost = Number(o.laborCost || o.labor_cost || 0);
  var ohCost  = Number(o.overheadCost || o.overhead_cost || 0);
  var totalCost = matCost + labCost + ohCost;
  var unitCost = produced > 0 ? (totalCost / produced) : 0;

  var statusMap = {
    planned:['#94a3b8','مخطط'], released:['#f59e0b','مُطلق'],
    in_progress:['#06b6d4','قيد التنفيذ'], completed:['#22c55e','مكتمل'],
    closed:['#16a34a','مُقفل'], cancelled:['#ef4444','ملغي']
  };
  var st = statusMap[o.status] || ['#64748b', o.status];

  // Hero header with progress
  var html =
    '<div style="background:linear-gradient(135deg,#1e293b,#334155);color:#fff;border-radius:14px;padding:18px;margin-bottom:14px;">' +
      '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">' +
        '<div style="flex:1;">' +
          '<div style="font-size:11px;color:#94a3b8;letter-spacing:.05em;">أمر إنتاج · ' + _v3EscapeHtml(o.id) + '</div>' +
          '<div style="font-size:20px;font-weight:900;color:#fff;margin-top:4px;">' + _v3EscapeHtml(o.productName || o.product_name || '—') + '</div>' +
          '<div style="font-size:11px;color:#cbd5e1;margin-top:6px;">' +
            '<i class="fas fa-tags"></i> ' + _v3EscapeHtml(o.brandName || o.brand_id || '—') + ' · ' +
            '<i class="fas fa-warehouse"></i> ' + _v3EscapeHtml(o.warehouseName || o.warehouse_id || '—') + ' · ' +
            '<i class="far fa-calendar"></i> ' + (o.orderDate || o.order_date ? new Date(o.orderDate || o.order_date).toLocaleDateString('ar-SA') : '—') +
          '</div>' +
        '</div>' +
        '<span style="background:'+st[0]+';color:#fff;padding:5px 12px;border-radius:999px;font-size:11px;font-weight:800;">' + st[1] + '</span>' +
      '</div>' +
      // Progress bar
      '<div style="margin-top:14px;">' +
        '<div style="display:flex;justify-content:space-between;font-size:11px;color:#cbd5e1;margin-bottom:4px;">' +
          '<span>الإنتاج</span><span><b style="color:'+pctClr+';font-size:14px;">' + _v3Fmt(produced) + '</b> / ' + _v3Fmt(planned) + ' (' + pct + '%)</span>' +
        '</div>' +
        '<div style="background:#0f172a;border-radius:999px;height:10px;overflow:hidden;">' +
          '<div style="height:100%;background:linear-gradient(90deg,'+pctClr+',#22c55e);width:' + pct + '%;transition:width .4s;"></div>' +
        '</div>' +
        (scrap > 0 ? '<div style="font-size:11px;color:#fca5a5;margin-top:4px;"><i class="fas fa-triangle-exclamation"></i> هدر: ' + _v3Fmt(scrap) + '</div>' : '') +
      '</div>' +
    '</div>';
  ...
  // Consumption table (raw materials used)
  html += '<div style="background:#fff;border:1.5px solid #e2e8f0;border-radius:12px;padding:12px;margin-bottom:14px;">' +
    '<div style="font-size:13px;font-weight:900;color:#1e293b;margin-bottom:8px;display:flex;align-items:center;gap:6px;"><i class="fas fa-box-open" style="color:#3b82f6;"></i> المكونات المستهلكة (' + consumption.length + ')</div>';
  if (consumption.length) {
    html += '<table class="wo-table" style="font-size:12px;"><thead><tr><th>المكوّن</th><th class="num">الكمية</th><th>الوحدة</th><th class="num">التكلفة</th></tr></thead><tbody>' +
      consumption.map(function(c) {
        return '<tr>' +
          '<td>' + _v3EscapeHtml(c.itemName || c.item_name || c.itemId || '') + '</td>' +
          '<td class="num">' + _v3Fmt(c.quantity || c.qty || 0) + '</td>' +
          '<td>' + _v3EscapeHtml(c.unit || '') + '</td>' +
          '<td class="num" style="font-weight:700;color:#1e40af;">' + _v3Fmt(c.totalCost || c.total_cost || (Number(c.quantity||0) * Number(c.unitCost||c.unit_cost||0))) + ' ر.س</td>' +
        '</tr>';
      }).join('') + '</tbody></table>';
  } else {
    html += '<div style="text-align:center;color:#94a3b8;padding:18px;font-size:12px;"><i class="fas fa-box" style="font-size:24px;display:block;margin-bottom:6px;"></i>' +
      (o.status === 'planned' ? 'سيتم خصم المكونات عند الإطلاق' : 'لا توجد مكوّنات مسجّلة') + '</div>';
  }
  html += '</div>';

  // Output table (finished goods received)
  if (output.length) {
    html += '<div style="background:#fff;border:1.5px solid #e2e8f0;border-radius:12px;padding:12px;margin-bottom:14px;">' +
      '<div style="font-size:13px;font-weight:900;color:#1e293b;margin-bottom:8px;display:flex;align-items:center;gap:6px;"><i class="fas fa-boxes-stacked" style="color:#16a34a;"></i> الإنتاج المستلَم (' + output.length + ')</div>' +
      '<table class="wo-table" style="font-size:12px;"><thead><tr><th>المنتج</th><th class="num">الكمية</th><th class="num">التكلفة</th><th>التاريخ</th></tr></thead><tbody>' +
      output.map(function(out) {
        return '<tr>' +
          '<td>' + _v3EscapeHtml(out.itemName || out.item_name || '') + '</td>' +
          '<td class="num" style="font-weight:700;">' + _v3Fmt(out.quantity || out.qty || 0) + '</td>' +
          '<td class="num">' + _v3Fmt(out.totalCost || out.total_cost || 0) + '</td>' +
          '<td style="font-size:11px;color:#64748b;">' + (out.createdAt || out.created_at ? new Date(out.createdAt || out.created_at).toLocaleString('ar-SA',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}) : '—') + '</td>' +
        '</tr>';
      }).join('') + '</tbody></table>' +
    '</div>';
  }

  // GL journals if posted
  if (o.glReleaseId || o.gl_release_id || o.glCompleteId || o.gl_complete_id) {
    html += '<div style="background:#dbeafe;border:1.5px solid #93c5fd;border-radius:12px;padding:12px;margin-bottom:14px;">' +
      '<div style="font-size:13px;font-weight:900;color:#1e40af;margin-bottom:6px;"><i class="fas fa-link" style="margin-left:4px;"></i> القيود المحاسبية المرتبطة</div>' +
      (o.glReleaseId || o.gl_release_id ? '<div style="font-size:12px;color:#1e3a8a;">قيد الإطلاق (خصم المواد): <code>' + _v3EscapeHtml(o.glReleaseId || o.gl_release_id) + '</code></div>' : '') +
      (o.glCompleteId || o.gl_complete_id ? '<div style="font-size:12px;color:#1e3a8a;">قيد الإكمال (إضافة المنتج): <code>' + _v3EscapeHtml(o.glCompleteId || o.gl_complete_id) + '</code></div>' : '') +
    '</div>';
  }

  // Notes
  if (o.notes) {
    html += '<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:10px 12px;margin-bottom:12px;font-size:12px;color:#78350f;line-height:1.6;"><b>ملاحظات:</b> ' + _v3EscapeHtml(o.notes) + '</div>';
  }

  // Footer actions
  var footer = '<button class="wo-btn wo-btn-secondary" onclick="WoModal.close()">إغلاق</button>';
  if (o.status === 'planned') {
    footer = '<button class="wo-btn wo-btn-secondary" onclick="WoModal.close()">إغلاق</button>' +
             '<button class="wo-btn wo-btn-warning" onclick="WoModal.close();erpReleaseProduction(\''+o.id+'\')"><i class="fas fa-play"></i> إطلاق الأمر</button>';
  } else if (o.status === 'released' || o.status === 'in_progress') {
    footer = '<button class="wo-btn wo-btn-secondary" onclick="WoModal.close()">إغلاق</button>' +
             '<button class="wo-btn wo-btn-success" onclick="WoModal.close();erpCompleteProduction(\''+o.id+'\')"><i class="fas fa-check"></i> إكمال الإنتاج</button>';
  }

  WoModal.open({
    icon: 'fa-industry', iconColor: st[0],
...
function _v3CostCard(icon, label, value, color) {
  return '<div style="background:#fff;border:1.5px solid '+color+'30;border-radius:10px;padding:10px 12px;">' +
    '<div style="font-size:10px;color:#64748b;font-weight:700;display:flex;align-items:center;gap:4px;"><i class="fas '+icon+'" style="color:'+color+';"></i> ' + label + '</div>' +
    '<div style="font-size:15px;font-weight:900;color:'+color+';margin-top:4px;">' + _v3Fmt(value) + ' ر.س</div>' +
  '</div>';
}
    ...
    .then(function(rows) {
      ...
      listEl.innerHTML = rows.map(function(r) {
        ...
        return '<div style="display:flex;gap:10px;padding:12px;background:#fff;border:1px solid #e5e7eb;border-right:3px solid '+rs.side+';border-radius:12px;margin-bottom:10px;'+(isMine ? 'background:linear-gradient(180deg,#fafbff 0%,#fff 100%);' : '')+'box-shadow:0 1px 3px rgba(15,23,42,.04);">' +
          '<div style="width:40px;height:40px;border-radius:50%;background:'+rs.grad+';color:#fff;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-weight:900;font-size:13px;letter-spacing:-0.5px;box-shadow:0 2px 6px rgba(0,0,0,.1);">' + esc(initials(r.authorName||r.authorUsername)) + '</div>' +
          '<div style="flex:1;min-width:0;">' +
            '<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">' +
              '<span style="font-weight:900;font-size:13px;color:#0f172a;">' + esc(r.authorName||r.authorUsername||'') + '</span>' +
              ...
              '<span style="font-size:10px;font-weight:800;color:#fff;background:'+rs.side+';padding:2px 8px;border-radius:6px;"><i class="fas fa-user-tag" style="font-size:8px;margin-inline-end:3px;"></i>' + esc(posLbl) + '</span>' +
            ...
            '</div>' +
            '<div style="font-size:13.5px;color:#334155;line-height:1.75;margin-top:6px;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;background:#f8fafc;padding:10px 12px;border-radius:8px;border:1px solid #f1f5f9;">' + esc(r.replyText||'') + '</div>' +
            att +
          '</div>' +
        '</div>';
      }).join('');
