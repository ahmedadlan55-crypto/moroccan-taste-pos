...
export function KpiGrid({ data }: { data: DashboardOverview }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <KpiTile label="المبيعات" value={formatCurrency(k.sales?.value ?? 0)} delta={k.sales?.delta} />
      <KpiTile label="عدد الطلبات" value={formatNumber(k.orders?.value ?? 0)} delta={k.orders?.delta} />
      <KpiTile label="متوسط الفاتورة" value={formatCurrency(k.avgTicket?.value ?? 0)} delta={k.avgTicket?.delta} />
      <KpiTile label="صافي الدخل" value={formatCurrency(k.netIncome?.value ?? 0)} delta={k.netIncome?.delta} />
      <KpiTile label="المصروفات" value={formatCurrency(k.expenses?.value ?? 0)} delta={k.expenses?.delta} />
      <KpiTile label="المشتريات" value={formatCurrency(k.purchases?.value ?? 0)} delta={k.purchases?.delta} />
      <KpiTile label="هامش الربح الإجمالي" value={`${formatNumber(gm)}%`} />
    </div>
  );
}
  ...
  const tile = (label: string, v?: OpsValue) => (
    <div className="surface p-4">
      ...
      <div className="mt-0.5 text-[11px] font-bold text-slate-400" dir="ltr">
        {formatNumber(v?.count ?? 0)} عنصر
      </div>
    </div>
  );
  return (
    <div className="grid gap-4 sm:grid-cols-3">
      {tile("مدفوعات معلّقة", ops.pendingPayments)}
      {tile("ذمم مدينة مستحقة", ops.arOutstanding)}
      {tile("ذمم دائنة مستحقة", ops.apOutstanding)}
    </div>
  );
}
