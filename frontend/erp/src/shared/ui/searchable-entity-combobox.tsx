// The ONE reusable searchable picker for every long list (items, GL accounts,
// suppliers, warehouses, customers, users, …). Replaces every large native
// <select> and every "load everything then filter" list.
//
// Guarantees:
//   • opens with the FIRST PAGE (searchOnEmpty=true) so allowed options show
//     before the user types
//   • server-side search — the fetcher hits a paginated `?q=…&page=…` endpoint
//   • debounce (250ms), previous request auto-aborted
//   • infinite scroll via useInfiniteQuery + a scroll sentinel
...
//   • the popup renders in a PORTAL, fixed-positioned above dialogs/drawers and
//     flips UP when there is no room below
...
export function SearchableEntityCombobox<T>(props: SearchableEntityComboboxProps<T>) {
  ...
  // close on outside click — but NOT inside the portal list
  useEffect(() => {
  ...
  // keep the active row within the visible window
    ...
    return (
      <div className="flex min-h-11 items-center justify-between gap-2 rounded-xl border border-teal-200 bg-teal-50/60 px-3 py-2">
        <span className="flex min-w-0 flex-col">
          ...
          {sub && (
            <span dir="ltr" className="truncate text-[11px] font-semibold text-slate-500 tabular-nums">
              {sub}
            </span>
          )}
        </span>
        {!disabled && (
          <button
            type="button"
            className="grid h-11 w-11 shrink-0 place-items-center rounded-xl text-slate-400 transition hover:bg-white hover:text-rose-600 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-teal-100"
