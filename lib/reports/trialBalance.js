 ...
 *
 * CONTRACT — fixed CO-MAIN ledger. gl_accounts does carry company_id, but the
 * JWT has no trustworthy company claim and the shell selector is not an
 * authorization boundary. Every financial figure is therefore joined to a
 * CO-MAIN account; callers cannot select another company. NULL is treated as
 * legacy CO-MAIN data until migration 0032 makes ownership NOT NULL.
 *
 ...
 *
 * PACKAGE G fix in this revision:
 *   - CONTRA ACCOUNTS ARE NO LONGER "ABNORMAL". `abnormalSign` was derived
 *     from `type` alone — asset/expense expect a debit, everything else a
 *     credit — and this engine had no concept of a contra account. So
 *     accumulated depreciation (an ASSET with a natural CREDIT balance),
 *     owner drawings (EQUITY, natural DEBIT) and the allowance for doubtful
 *     debts were flagged abnormal on every single run and counted in
 *     totals.abnormalCount. A permanently non-zero anomaly counter is a
 *     counter nobody reads, which is how a real abnormal balance would have
 *     hidden in plain sight. The expected side now comes from
 *     lib/coa/classify.js: gl_accounts.normal_balance, FLIPPED when
 *     gl_accounts.is_contra is set. An asset with a credit balance that is
 *     NOT contra is still abnormal — that check is unchanged, and it is the
 *     one that matters.
 *   - The debit/credit COLUMN split is untouched: a trial balance shows each
 *     balance on the side it actually falls, and the footer must still tie.
 *     Only the anomaly FLAG changed.
 *   - New `unmapped` array: accounts whose normal balance or contra status
 *     could not be read from stored metadata and fell back to `type`.
 *     Empty is the only honest way to claim the chart carries its own
 *     classification.
 *   - Inclusion policy is DELIBERATELY unchanged: this report includes every
 *     account regardless of is_active/status (that is what makes it the
 *     ledger's ground truth), unlike the balance sheet / cash flow / income
 *     statements which show archived accounts only for periods they moved in.
 *
...
const classify = require('../coa/classify');
...
// routes/erp-core.js, using lib/warehouseScope.js#hasWarehouseAccess
// directly against req.warehouseScope (always enforced for this report,
// regardless of the WAREHOUSE_SCOPE_ENFORCE shadow-mode flag other,
// non-financial warehouse-ops routes honor — see the Tier A.3 comment at
// the route for why this report can't inherit that rollout-friendly
// default). `branch` has NO equivalent access-control infrastructure
// anywhere in this codebase (no user_branch_access table, no
// req.guardBranch) — inventing one would mean guessing both a new schema
// AND a new authorization default (does "accountant" mean "restricted to
// one branch" or "sees the whole consolidated ledger"? this repo has no
// answer on record, and guessing wrong is worse than refusing). Tier A.3
// Release Gate item 7 — REFUSED explicitly instead: `branch` is always
// rejected as unsupported, the same SCHEMA_NOT_READY/409 contract already
// used for a genuinely-missing column, so "arbitrary branch reads" is
// categorically impossible rather than merely undocumented.
function assertDimensionsSupported(dim, { branch, brand, costCenter, warehouse }) {
  if (branch) {
    throw new TrialBalanceError(
      'تصفية حسب الفرع (branch) غير مدعومة حاليًا لهذا التقرير — لا توجد بنية تحقق صلاحية نطاق الفرع بعد؛ ' +
      'السماح بالتصفية دون تحقق صلاحية يعني إمكانية قراءة بيانات أي فرع تعسفًا، وهو مرفوض صراحة.',
      'TB_BRANCH_SCOPE_NOT_SUPPORTED',
      409
    );
  }
  const missing = [];
...
// Cycle-safe 1-based tree depth. This implementation MOVED to lib/coa/tree.js
// — verbatim, not rewritten — and is re-exported here so every existing caller
// and test keeps working while the chart-of-accounts screens, the reports and
// the repair endpoints all start answering "how deep is this account?" from one
// place. It was already the correct one (cycle-safe, 1-based, hard-capped), so
// promoting it to the shared module is what makes the OTHER copies deletable
// without any risk to this engine's arithmetic.
//
// The bug it had to grow to fix, recorded so it is not re-introduced: a plain
// A<->B cycle used to flag only ONE of the two nodes, because the walk kept
// `pathSet` (a Set, no ordering) and only the single revisited id was added to
// cycleMembers. Tracking the ancestry as an ORDERED array too means every node
// from the repeated id's first occurrence onward is marked. A node that merely
// points INTO a cycle is still correctly excluded.
const computeDepths = coaTree.computeDepths;
...
async function computeTrialBalance(db, opts) {
  ...
  if (o.companyId != null && String(o.companyId) !== LEDGER_COMPANY_ID) {
    throw new TrialBalanceError(
      'ميزان المراجعة متاح حاليًا لدفتر CO-MAIN فقط',
      'TB_COMPANY_SCOPE_FIXED',
      400
    );
  }
  ...
  const { clause: openBoundaryClause, params: openBoundaryParams } = buildWhere([
    ...
    { sql: 'j.id <> ?', params: [COA_TRANSITION_JOURNAL_ID] },
  ...
  const [openRows] = await db.query(
    `SELECT COALESCE(coa_map.target_account_id, e.account_id) AS account_id,
            COALESCE(SUM(e.debit),0) d, COALESCE(SUM(e.credit),0) c
     FROM gl_entries e JOIN gl_journals j ON e.journal_id = j.id
     LEFT JOIN coa_0036_account_map coa_map ON coa_map.source_account_id = e.account_id
     JOIN gl_accounts scope_a ON scope_a.id = COALESCE(coa_map.target_account_id, e.account_id)
       AND COALESCE(scope_a.company_id, 'CO-MAIN') = ?
     WHERE e.account_id IS NOT NULL AND (${openBoundaryClause})
     GROUP BY COALESCE(coa_map.target_account_id, e.account_id)`,
    [LEDGER_COMPANY_ID, ...openBoundaryParams]
  );
  const openingMap = {};
  ...
  const [[fut]] = await db.query(
    `SELECT COUNT(DISTINCT j.id) n, COALESCE(SUM(e.debit),0) d, COALESCE(SUM(e.credit),0) c
     FROM gl_entries e JOIN gl_journals j ON e.journal_id = j.id
     JOIN gl_accounts scope_a ON scope_a.id = e.account_id
       AND COALESCE(scope_a.company_id, 'CO-MAIN') = ?
     WHERE ${futClause} AND e.account_id IS NOT NULL`,
    [LEDGER_COMPANY_ID, ...futParams]
  );
  ...
  const [periodRows] = await db.query(
    `SELECT COALESCE(coa_map.target_account_id, e.account_id) AS account_id,
     ...
     GROUP BY COALESCE(coa_map.target_account_id, e.account_id)`,
    [LEDGER_COMPANY_ID, ...periodBoundaryParams]
  );
  const periodMap = {};
  ...
  // Canonical display order — the same expression the chart-of-accounts screen
  // uses (lib/coa/tree.js ORDER_BY). It used to be a bare `ORDER BY code`,
  // which sorts a VARCHAR lexicographically: with variable-length codes that
  // puts `112 المخزون` between `1110` and `1120`, so the trial balance and the
  // CoA screen physically listed accounts in different sequences. Same rows,
  // same numbers, same totals — only the sequence changes.
  // PACKAGE G — project the stored classification columns. Without them the
  // classifier correctly reports every account as unmapped: a column missing
  // from the SELECT is indistinguishable from a column that is NULL, which is
  // exactly why every classify.js result carries its provenance.
  const [accounts] = await db.query(
    `SELECT id, code, name_ar, name_en, type, parent_id, is_folder, is_active, display_order,
            status, normal_balance, is_contra, report_section, level AS storedLevel
     FROM gl_accounts a
  ...
  const rows = accounts.map((a) => {
    ...
    // PACKAGE G — the side this balance is EXPECTED to sit on, from stored
    // metadata: normal_balance, flipped when is_contra. `type` is consulted
    // only when normal_balance is NULL, and that fallback is reported in
    // `unmapped` below rather than passing for stored data.
    const nb = classify.normalBalanceOf(a);
    // effectiveContraOf, not contraOf: is_contra is NOT NULL DEFAULT 0, so an
    // account created after 0028 declaring report_section='drawings' arrives
    // with the column at 0. Reading the column alone would flag it abnormal
    // on every run again — defect #3 through a data gap instead of a regex.
    const contra = classify.effectiveContraOf(a);
    const expected = classify.expectedSideOf(a);
    const isDebitNormal = nb.normalBalance !== 'credit';
    const expectsDebit = expected.expectedSide !== 'credit';

    // NOTE: naturalSide() decides which COLUMN a balance is printed in, and it
    // is driven by the account's CLASS normal balance, NOT by the contra flip.
    // Presentation is unchanged on purpose: the columns must keep tying to the
    // footer, and a contra account's balance already lands in the opposite
    // column by virtue of its sign. Only `abnormalSign` below uses the flip.
    ...
    // A contra account sitting on its own natural side is NOT an anomaly:
    // accumulated depreciation is an asset with a credit balance by design.
    // An asset with a credit balance that is NOT contra still is.
    const abnormalSign = expectsDebit ? closeCredit > 0.01 : closeDebit > 0.01;
    ...
    return {
      ...
      abnormalSign,
      // PACKAGE G — provenance travels with the row so a reader can tell
      // "classified" from "guessed" without re-deriving anything.
      isContra: contra.isContra,
      normalBalance: nb.normalBalance,
      expectedSide: expected.expectedSide,
      classificationSource: nb.source,
      journalCount: figs.journalCount,
      _classificationUnmapped: nb.unmapped || contra.unmapped,
      _normalBalanceSource: nb.source,
      _contraSource: contra.source,
  ...
  const [unbalancedJournalRows] = await db.query(
    `SELECT id, journal_number, journal_date, total_debit AS totalDebit, total_credit AS totalCredit
     ...
     WHERE j.status = 'posted' AND j.journal_date <= ?
       AND EXISTS (
         SELECT 1 FROM gl_entries se JOIN gl_accounts sa ON sa.id = se.account_id
          WHERE se.journal_id = j.id AND COALESCE(sa.company_id, 'CO-MAIN') = ?
       )
       AND ABS(j.total_debit - j.total_credit) > 0.01`,
    [to, LEDGER_COMPANY_ID]
  );
  const [headerLineMismatchRows] = await db.query(
    `SELECT j.id, j.journal_number, j.journal_date,
            ...
            COALESCE(SUM(CASE WHEN e.account_id IS NULL OR COALESCE(ea.company_id, 'CO-MAIN') = 'CO-MAIN' THEN e.debit ELSE 0 END),0) AS lineDebit,
            COALESCE(SUM(CASE WHEN e.account_id IS NULL OR COALESCE(ea.company_id, 'CO-MAIN') = 'CO-MAIN' THEN e.credit ELSE 0 END),0) AS lineCredit
     FROM gl_journals j LEFT JOIN gl_entries e ON e.journal_id = j.id
     LEFT JOIN gl_accounts ea ON ea.id = e.account_id
     WHERE j.status = 'posted' AND j.journal_date <= ?
       AND EXISTS (
         SELECT 1 FROM gl_entries se JOIN gl_accounts sa ON sa.id = se.account_id
          WHERE se.journal_id = j.id AND COALESCE(sa.company_id, 'CO-MAIN') = ?
  ...
  const cleanRows = filtered.map((r) => {
    const {
      _ownHasActivity, _own, _ownOpenDebit, _ownOpenCredit, _ownCloseDebit, _ownCloseCredit,
      _classificationUnmapped, _normalBalanceSource, _contraSource, ...rest
    } = r;
    return rest;
  });

  // PACKAGE G — accounts whose normal balance / contra status did NOT come
  // from stored metadata. Scoped to rows that actually appear in the report,
  // so it is a list the reader can act on rather than a chart-wide audit.
  const unmapped = filtered
    .filter((r) => r._classificationUnmapped)
    .map((r) => ({
      id: r.accountId,
      code: r.code,
      nameAr: r.nameAr,
      type: r.type,
      source: r._normalBalanceSource,
      contraSource: r._contraSource,
      reason:
        r._normalBalanceSource === classify.SOURCES.TYPE_FALLBACK
          ? 'normal balance derived from gl_accounts.type — normal_balance is NULL'
          : 'is_contra not available for this account',
    }));

  return {
    success: true,
    isClean,
    // PACKAGE G — see above. NOT folded into isClean: an un-migrated chart is
    // a metadata gap, not an arithmetic fault, and conflating the two would
    // make every historical report read «غير سليم» for a reason that has
    // nothing to do with whether the ledger balances.
    unmapped,
...
module.exports = { computeTrialBalance, getDimCols, resetDimColsCache, TrialBalanceError, LEDGER_COMPANY_ID };
