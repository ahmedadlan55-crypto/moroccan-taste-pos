...
describe("product-card-price + product-card-inline-add — the card sells", () => {
  it("each card shows the tax-inclusive price and the WHOLE card adds the item", () => {
    ...
    // 23.00 NET × 15% = 26.45, which the till now sells as a whole 26. This
    // assertion used to read "23.00" while its own title said "tax-inclusive" —
    // the card advertised the stored net for an item the cart rang up higher,
    // and a green test said that was correct. It was the owner's complaint,
    // encoded as a passing assertion.
    expect(card).toHaveTextContent("26");
    // And explicitly NOT the stored figure — the regression this pins is the
    // net price reappearing, which "26 is present" alone would not catch.
