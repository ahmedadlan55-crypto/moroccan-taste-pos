...
async function postCreditNote(conn, { ret, net, vat, cost = 0, warehouseId, revenueByAccount, cogsByWarehouse }) {
  ...
  if (c > 0) {
    ...
    cogsJournalId = await _post(conn, {
      journalDate: calc.ymd(ret.return_date),
  ...
  // Both ids: the COGS journal used to be posted and DISCARDED — no caller,
  // no response, no sales_returns column referenced it.
  return { journalId: jId, cogsJournalId };
}
