/**
 * lib/procurement/accounts.js — GRNI / AP / Input-VAT account bootstrap.
 *
...
function codes() {
  return {
    grni: String(process.env.PROCUREMENT_GRNI_ACCOUNT_CODE || '2150'),
    ap: String(process.env.PROCUREMENT_AP_ACCOUNT_CODE || '2100'),
    inputVat: String(process.env.PROCUREMENT_INPUT_VAT_ACCOUNT_CODE || '1290'),
  };
}
...
/**
 * Ensure the three accounts exist and are postable. Creates GRNI when absent.
 * Returns { grni, ap, inputVat } each { id, code }.  Throws on structural
 * problems. Idempotent.
 */
async function ensureProcurementAccounts(db, opts = {}) {
  ...
  // AP + Input VAT must pre-exist (seeded by ensureCoreAccounts).
