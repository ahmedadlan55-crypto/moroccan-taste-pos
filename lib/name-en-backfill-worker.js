 ...
 *           translation API. It resolves the name OFFLINE against the
 *           curated menu dictionary (lib/menu-en-dictionary.js) — real
 *           English for real menu vocabulary ("شاي بالنعناع" → "Mint
 *           Tea") — and falls back to a deterministic Arabic→Latin
 *           transliteration for anything the dictionary does not know.
 *           The result is labeled honestly ('machine_translation' when
 *           the dictionary resolved something, else 'transliteration')
 *           and always marked as needing human review.
...
const { translateMenuName } = require('./menu-en-dictionary');
...
async function _processOne(row) {
  ...
  // No translation API key is configured in this environment, and we never
  // invent/call one. Instead: the curated offline menu dictionary
  // (lib/menu-en-dictionary.js) turns real menu vocabulary into real English
  // ("شاي بالنعناع" → "Mint Tea"); any token it does not know falls back to the
  // deterministic transliteration below, so the result is always Latin script.
  // Bare transliteration alone produced Latin-but-not-English gibberish
  // ("'Syr Brtqal"), which is useless to a cashier reading the English UI.
  //
  // The label is honest about which path produced the name: 'machine_translation'
  // only when the dictionary actually resolved something, else 'transliteration'.
  // Either way name_en_needs_review stays 1 — the owner reviews in the ERP.
  const dict = translateMenuName(menu.name || '', transliterate);
  const usedDictionary = dict.matched > 0;
  const proposed = usedDictionary ? dict.text : transliterate(menu.name || '');
  const sourceUsed = usedDictionary ? 'machine_translation' : 'transliteration';
