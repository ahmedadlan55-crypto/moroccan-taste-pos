...
function buildQueueRow(evt) {
  ...
  return {
    ...
    payload_json: JSON.stringify({
      ...
      payments: Array.isArray(evt.payments) ? evt.payments : [],
      revenue: Array.isArray(evt.revenue) ? evt.revenue : [],
      cogsByWarehouse: Array.isArray(evt.cogsByWarehouse) ? evt.cogsByWarehouse : [],
...
async function capture(conn, evt, opts = {}) {
  ...
  try {
    ...
    // is expected during a rollout and must not take the till down.
