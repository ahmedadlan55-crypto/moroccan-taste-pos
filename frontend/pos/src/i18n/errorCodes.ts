...
/**
 * Translate an ApiError-shaped `unknown` (see lib/api.ts's ApiError: {status,
 * code, message}) into a user-facing string, so call sites stop doing
 * `pushToast("error", (e as Error).message)` with a raw, possibly-English,
 * possibly-technical server string.
 *
 * Resolution order:
