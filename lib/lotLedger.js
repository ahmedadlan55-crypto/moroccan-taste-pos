...
async function receiveInbound(conn, ctx) {
  ...
  if (EXP.ymd(lot.expiryDate)) {
    ...
    if (!rc.ok) throw _err('EXPIRED_LOT', 'لا يمكن استلام دفعة منتهية الصلاحية');
    if (rc.level === 'near' && !ctx.allowNearExpiry) throw _err('VALIDATION_ERROR', 'الدفعة قريبة الانتهاء (' + rc.daysUntil + ' يومًا) — يلزم تأكيد/سبب للاستلام');
    if (rc.level === 'near') expiryWarning = { level: 'near', daysUntil: rc.daysUntil };
  }
