...
// v6.19.1 — Legacy 'pending' status (written by TxnService.create) is
// treated as in_progress, mirroring the state machine fallback in
// routes/workflow.js ("Handle legacy 'pending' status by treating it as
// 'in_progress'"). Without this, approve/reject straight from a list row
// (incl. bulk actions) was denied until the assignee first OPENED the
// transaction (the detail view auto-advances pending → in_progress).
const ACTIONABLE_STATES = Object.freeze(
  [STATES.CREATED, STATES.IN_PROGRESS, STATES.REPLIED, 'pending']);
const FORWARDABLE_STATES = Object.freeze(
  [STATES.IN_PROGRESS, STATES.REPLIED, 'pending']);
...
const PERMISSIONS = {
  ...
  'txn.action.approve': {
    check: (user, ctx) => {
      const t = ctx.txn;
      if (!t) return false;
      if (!ACTIONABLE_STATES.includes(t.status)) return false;
  ...
  'txn.action.forward': {
    check: (user, ctx) => {
      const t = ctx.txn;
      if (!t) return false;
      if (!FORWARDABLE_STATES.includes(t.status)) return false;
      if (!_isAssignee(user, t)) return false;
      if (_actorAlreadyActedOnStage(user, ctx)) return false;
      return true;
    }
