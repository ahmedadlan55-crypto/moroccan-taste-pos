// Phase U — quantity entry in a chosen unit, with a live base-unit conversion.
// Two patterns (per the directive §رابعًا):
//   • "single"    — one unit <select> (base + majors — a SHORT per-item list, so
//     a native select is correct here) + a quantity, e.g. 3 كراتين = 36 حبة
//   • "composite" — major quantity + minor(base) quantity, e.g. 2 كرتون + 3 حبة
//     = 27 حبة (used mainly for stocktake counts)
...
// minorQty, baseQty } so the caller can build the document line payload.
...
export function baseFromValue(units: ItemUnitLite[], v: UnitQtyValue, mode: "single" | "composite"): number {
...
export function UnitQtyInput({
    ...
    return (
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-2">
          <div className="flex flex-col">
            <input
              ...
              className="field h-10 w-24 text-center tabular-nums"
              value={value.majorQty ?? ""}
              onChange={(e) => onChange({ ...value, unitCode: majorUnit?.code ?? value.unitCode, majorQty: e.target.value === "" ? 0 : Number(e.target.value) })}
              aria-label={`عدد ${majorUnit?.name ?? "الوحدة الكبرى"}`}
              dir="ltr"
            />
            <span className="mt-0.5 text-center text-[10px] font-bold text-slate-400">{majorUnit?.name ?? "كبرى"}</span>
          </div>
          ...
          <div className="flex flex-col">
            <input
              ...
              onChange={(e) => onChange({ ...value, minorQty: e.target.value === "" ? 0 : Number(e.target.value) })}
        ...
        <span className="text-[11px] font-bold text-teal-700" dir="rtl">
          = <span dir="ltr" className="tabular-nums">{formatQty(baseQty)}</span> {baseName}
          {mFactor > 1 && <span className="text-slate-400"> (1 {majorUnit?.name} = <span dir="ltr" className="tabular-nums">{formatQty(mFactor)}</span> {baseName})</span>}
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-2">
        <input
          type="number" min={minQty} step="any" disabled={disabled}
          className="field h-10 w-24 text-center tabular-nums"
          value={value.qty ?? ""}
        ...
        {units.length > 1 ? (
          <select
            className="field h-10 w-28" disabled={disabled}
            value={value.unitCode || base?.code || ""}
            onChange={(e) => onChange({ ...value, unitCode: e.target.value })}
            aria-label="الوحدة"
          >
            {units.map((u) => (
              <option key={u.code} value={u.code}>{u.name}{u.isBase ? "" : ` (×${formatQty(u.factor)})`}</option>
            ))}
          </select>
        ) : (
          <span className="grid h-10 place-items-center rounded-xl bg-slate-50 px-3 text-xs font-bold text-slate-500">{baseName}</span>
        )}
      </div>
      {factor > 1 && (
        <span className="text-[11px] font-bold text-teal-700" dir="rtl">
