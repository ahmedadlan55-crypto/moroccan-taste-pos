...
/**
 * The canonical DISPLAY ordering for accounts, as a SQL fragment.
 *
 *   1. `display_order` when set — this is the owner's explicit intent, and the
 *      only thing that makes a reorder UI meaningful. NULLs sort last via a
 *      sentinel rather than `IS NULL` so the expression stays index-friendly.
 *   2. `CHAR_LENGTH(code)` — this is what stops `1110 < 112 < 1120`. Within a
 *      sibling group codes share a length, so this never reorders true
 *      siblings; it only stops a SHORTER code from landing in the middle of a
 *      longer group when a query flattens the tree.
 *   3. `code` — the final, total tiebreak.
 *
...
function ORDER_BY(alias = 'a') {
  const a = alias ? alias + '.' : '';
  return `COALESCE(${a}display_order, 2147483647), CHAR_LENGTH(${a}code), ${a}code`;
}
...
/** The JS twin of ORDER_BY(), for sorting rows already in memory. */
