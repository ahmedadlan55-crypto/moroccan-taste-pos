...
async function assertInvariant(conn, warehouseId, itemId) {
  ...
  if (Math.abs(lotSum - stockQty) > 0.001) {
    // Data-integrity ALERT — count + emit a loud structured log before failing
    // the transaction (which rolls back; nothing is persisted out of balance).
    try { require('./v2Metrics').inc('lot_invariant_violation_total'); } catch (_) {}
    try { require('./logger').fatal({ alert: 'lot_invariant_violation', warehouseId, itemId, lotSum, stockQty }, 'LOT_INVARIANT_VIOLATION'); } catch (_) {}
...
// ── TRANSFER: issue allocates source lots (FEFO) into an in-transit ledger ──
// ctx: { transferId, sourceWh, destWh, itemId, qty, trackingMode, movementSeq,
//        movementId, actor, occurredAt, now }
async function transferIssue(conn, ctx) {
  const out = await allocateOutbound(conn, {
    warehouseId: ctx.sourceWh, itemId: ctx.itemId, qty: ctx.qty, trackingMode: ctx.trackingMode,
    movementSeq: ctx.movementSeq, movementId: ctx.movementId, referenceType: 'transfer_out', referenceId: ctx.transferId,
    reason: 'تحويل صادر', actor: ctx.actor, occurredAt: ctx.occurredAt, now: ctx.now,
  });
  for (const a of out.allocations) {
    await conn.query(
      'INSERT INTO lot_transfer_allocations (id, transfer_id, item_id, source_lot_id, lot_number, expiry_date, source_warehouse_id, dest_warehouse_id, qty, received_qty) VALUES (?,?,?,?,?,?,?,?,?,0) ' +
      'ON DUPLICATE KEY UPDATE qty = qty + VALUES(qty)',
      [_id('LTA'), ctx.transferId, ctx.itemId, a.lotId, a.lotNumber, a.expiryDate, ctx.sourceWh, ctx.destWh, round3(a.qty)]
    );
  }
  return out;
}

// ── TRANSFER: receive (partial-safe) adds the SAME lots to the destination ──
// Pulls from the transfer's in-transit allocations (earliest-expiry first) up to qty.
async function transferReceive(conn, ctx) {
  const [allocs] = await conn.query(
    'SELECT * FROM lot_transfer_allocations WHERE transfer_id=? AND item_id=? AND received_qty < qty ORDER BY (expiry_date IS NULL), expiry_date, source_lot_id FOR UPDATE',
    [ctx.transferId, ctx.itemId]
  );
  let remaining = round3(ctx.qty);
  const received = [];
  for (const al of allocs) {
    if (remaining <= 0) break;
    const avail = round3(Number(al.qty) - Number(al.received_qty));
    if (avail <= 0) continue;
    const take = Math.min(avail, remaining);
    await _adjustBalance(conn, ctx.destWh, ctx.itemId, al.source_lot_id, round3(take));
    await _insertLotMovement(conn, { movementSeq: ctx.movementSeq, movementId: ctx.movementId, lotId: al.source_lot_id, warehouseId: ctx.destWh, itemId: ctx.itemId, signedQty: round3(take), referenceType: 'transfer_in', referenceId: ctx.transferId, reason: 'تحويل وارد', actor: ctx.actor, occurredAt: ctx.occurredAt });
    await conn.query('UPDATE lot_transfer_allocations SET received_qty = received_qty + ? WHERE id=?', [round3(take), al.id]);
    received.push({ lotId: al.source_lot_id, lotNumber: al.lot_number, qty: round3(take) });
    remaining = round3(remaining - take);
  }
  if (remaining > 0.001) throw _err('INSUFFICIENT_LOT_QUANTITY', 'كمية الاستلام تتجاوز المتبقي في التحويل (' + remaining + ')');
  return { received };
}

// ── TRANSFER: reverse — return issued lots to source, remove received from dest ──
async function transferReverse(conn, ctx) {
  const [allocs] = await conn.query('SELECT * FROM lot_transfer_allocations WHERE transfer_id=? AND item_id=?', [ctx.transferId, ctx.itemId]);
  for (const al of allocs) {
    const issued = round3(Number(al.qty));
    const rec = round3(Number(al.received_qty));
    if (issued > 0) { // restore to source
      await _adjustBalance(conn, ctx.sourceWh, ctx.itemId, al.source_lot_id, issued);
      await _insertLotMovement(conn, { movementSeq: ctx.movementSeq, movementId: ctx.movementId, lotId: al.source_lot_id, warehouseId: ctx.sourceWh, itemId: ctx.itemId, signedQty: issued, referenceType: 'transfer_out_reverse', referenceId: ctx.transferId, reason: 'عكس تحويل (مصدر)', actor: ctx.actor, occurredAt: ctx.occurredAt });
    }
    if (rec > 0) { // remove from destination
      await _adjustBalance(conn, ctx.destWh, ctx.itemId, al.source_lot_id, -rec);
      await _insertLotMovement(conn, { movementSeq: ctx.movementSeq, movementId: ctx.movementId, lotId: al.source_lot_id, warehouseId: ctx.destWh, itemId: ctx.itemId, signedQty: -rec, referenceType: 'transfer_in_reverse', referenceId: ctx.transferId, reason: 'عكس تحويل (وجهة)', actor: ctx.actor, occurredAt: ctx.occurredAt });
    }
  }
  return { reversed: allocs.length };
}

// ── STOCKTAKE: reconcile a tracked item's lot balances to the counted values ──
// ctx: { warehouseId, itemId, countedLots:[{lotId?, lotNumber?, expiryDate?, qty}],
//        referenceId, actor, occurredAt }. Every existing lot is set to its counted
// value (0 if not in the count); counted lots with no balance row are added; an
// "unknown" lot (no lotId) is created. After this, Σ(lot)=warehouse_stock.qty.
async function reconcileStocktakeLots(conn, ctx) {
  const balances = await loadBalances(conn, ctx.warehouseId, ctx.itemId, true);
  const counted = new Map();
  const unknownLots = [];
  for (const cl of (ctx.countedLots || [])) {
    if (cl.lotId) counted.set(cl.lotId, round3(cl.qty));
    else unknownLots.push(cl);
  }
  const move = (lotId, signed, reason) => _insertLotMovement(conn, { movementSeq: ctx.movementSeq || null, movementId: ctx.movementId || null, lotId, warehouseId: ctx.warehouseId, itemId: ctx.itemId, signedQty: signed, referenceType: 'stocktake', referenceId: ctx.referenceId, reason, actor: ctx.actor, occurredAt: ctx.occurredAt });
  // 1) bring each EXISTING balance to its counted value (0 when not counted)
  for (const b of balances) {
    const target = counted.has(b.lotId) ? counted.get(b.lotId) : 0;
    const delta = round3(target - b.qty);
    if (Math.abs(delta) > 0.0001) { await _adjustBalance(conn, ctx.warehouseId, ctx.itemId, b.lotId, delta); await move(b.lotId, delta, 'تسوية جرد للدفعة'); }
    counted.delete(b.lotId);
  }
  // 2) counted KNOWN lots that had no balance row yet → add
  for (const [lotId, qty] of counted) {
    if (qty > 0) { await _adjustBalance(conn, ctx.warehouseId, ctx.itemId, lotId, qty); await move(lotId, qty, 'تسوية جرد (دفعة قائمة)'); }
  }
  // 3) UNKNOWN lots found during the count → create + add (audited workflow row)
  for (const cl of unknownLots) {
    if (round3(cl.qty) <= 0) continue;
    const { lotId } = await _findOrCreateLot(conn, ctx.itemId, { lotNumber: cl.lotNumber || ('UNKNOWN-' + ctx.referenceId), expiryDate: cl.expiryDate || null, sourceType: 'stocktake', sourceId: ctx.referenceId }, ctx.actor);
    await _adjustBalance(conn, ctx.warehouseId, ctx.itemId, lotId, round3(cl.qty));
    await move(lotId, round3(cl.qty), 'دفعة مجهولة مكتشفة بالجرد');
  }
  await assertInvariant(conn, ctx.warehouseId, ctx.itemId);
}
...
module.exports = {
  ...
  transferIssue, transferReceive, transferReverse, reconcileStocktakeLots,
};
