 ...
 *   - company_id is now NOT NULL (migration 0015) — getAccountByRole no
 *     longer has a "global (company_id IS NULL) fallback", which used to
 *     be a silent-fallback risk of its own kind ("لا Global fallback مالي
 *     صامت عند إرسال Company ID"). Every lookup is company-scoped for real.
...
async function getAccountByRole(db, roleKey, opts) {
  ...
  return { accountId: row.account_id, code: row.code, nameAr: row.name_ar, type: row.type };
}
