// Self-service clock-in/out helpers — a faithful port of the original employee
...
//
// Geofence rule (mirrors legacy): location is REQUIRED. The original app never
...
// because a clock row with no position cannot be geofence-checked afterwards.
//
// This file is shared, character for character, with the ERP's copy at
// frontend/erp/src/modules/people/lib/selfService.ts (which drives the same
// clock card inside the back-office). Keep the two in step: they encode one
// server contract, and the ERP copy carries the unit tests for the device
// fingerprint and the clock-state derivation.
