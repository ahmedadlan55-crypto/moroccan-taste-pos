...
async function runOnce() {
  ...
  // 3b. Project the valued movement ledger.
  //
  // It rides this worker rather than the 36 places that write an inventory
  // movement: completeness is the ledger's entire value, and a ledger that
  // depends on every future contributor remembering will eventually have
  // holes it cannot report. Replay is idempotent, so a missed tick is caught
  // up by the next one and a crash mid-batch costs nothing.
  try {
    await require('../inventoryValueLedger').runProjector(db);
  } catch (e) {
    console.warn('[inventory-value-ledger] projector error:', e && e.message);
  }
