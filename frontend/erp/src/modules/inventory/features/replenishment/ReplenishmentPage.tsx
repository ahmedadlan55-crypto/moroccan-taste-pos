...
import { fetchReplenishmentSnapshot, useReplenishment, useReplenishmentSummary } from "@/modules/inventory/lib/hooks/useReplenishment";
...
export function ReplenishmentPage() {
  ...
  const [reportRows, setReportRows] = useState<ReplenishmentRow[] | null>(null);
  ...
  const visibleRows = reportRows ?? data?.rows ?? [];

  useEffect(() => {
    if (reportAction !== "print" || reportRows === null) return;
    const clear = () => { setReportRows(null); setReportAction(null); };
  ...
  }, [reportAction, reportRows]);

  async function prepareReport(action: "print" | "csv") {
    setExpErr(null);
    setReportAction(action);
    try {
      const snapshot = await fetchReplenishmentSnapshot({ q, warehouseId, status, risk });
      if (action === "print") setReportRows(snapshot.rows);
      else {
        await exportCsv({ ...(warehouseId ? { warehouseId } : {}), ...(status ? { status } : {}), ...(risk ? { risk } : {}), ...(q ? { q } : {}) });
        setReportAction(null);
      }
    } catch {
      setReportRows(null); setReportAction(null);
      setExpErr(t("inventoryRest.replenishment.exportFailed"));
    }
  }
    ...
    <PrintDocument title={t("inventoryRest.replenishment.title")}>
      ...
      {showHow && (
        <div className="no-print mb-4 rounded-2xl border border-blue-100 bg-blue-50/60 p-4 text-sm text-blue-900">
          ...
          <ul className="list-disc space-y-1 ps-5 text-xs font-medium">
      ...
      <section className="no-print surface mt-4 flex flex-col gap-3 p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <label className="relative flex-1"><Search className="pointer-events-none absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <input className="field w-full ps-10" placeholder={t("inventoryRest.replenishment.searchPlaceholder")} defaultValue={q} onChange={(e) => patch({ q: e.target.value })} aria-label={t("common.search")} /></label>
      ...
      <section className="mt-4">
        {isLoading ? <LoadingState /> : isError ? <ErrorState error={error} onRetry={() => refetch()} /> : visibleRows.length === 0 ? (
            ...
            <div className="surface hidden overflow-x-auto md:block">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-xs font-bold text-slate-500"><tr>
                  <th className="px-3 py-3 text-start">{t("inventoryRest.replenishment.col.item")}</th><th className="px-3 py-3 text-start">{t("inventoryRest.replenishment.col.warehouse")}</th><th className="px-3 py-3">{t("inventoryRest.replenishment.col.onHand")}</th><th className="px-3 py-3">{t("inventoryRest.replenishment.col.reorderPoint")}</th>
