    ...
    <div>
      ...
      <section className="surface mt-4 flex flex-col gap-3 p-4">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
          ...
          <select className="field xl:w-48" value={fromWh} onChange={(e) => patch({ from: e.target.value })} aria-label="المستودع المصدر">
            <option value="">كل المصادر</option>
            {whOptions.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>
          <select className="field xl:w-48" value={toWh} onChange={(e) => patch({ to: e.target.value })} aria-label="المستودع الوجهة">
            <option value="">كل الوجهات</option>
            {whOptions.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>
          <input type="date" className="field xl:w-40" value={dateFrom} onChange={(e) => patch({ dateFrom: e.target.value })} aria-label="من تاريخ" />
          <input type="date" className="field xl:w-40" value={dateTo} onChange={(e) => patch({ dateTo: e.target.value })} aria-label="إلى تاريخ" />
        </div>
