...
function notFoundHandler(req, res) {
  res.status(404).json({
    success: false,
    // scrubUrl: never reflect credential-looking query params back to clients/logs
    error: 'المسار غير موجود: ' + req.method + ' ' + require('./urlScrub').scrubUrl(req.originalUrl),
    code: 'NOT_FOUND'
  });
}
