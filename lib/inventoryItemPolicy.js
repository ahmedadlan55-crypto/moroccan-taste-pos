...
// Guard for a LEGACY inbound/set path that cannot capture lot identity/expiry
// (legacy PO receive, legacy bulk add/stocktake-set). Blocks inactive items
// (deactivation can't be bypassed) AND tracked items (which must be received via
// the warehouse-v2 receipt so a proper lot is created — Phase 4B gate). Pass the
// item row (must include tracking_mode, active, deleted_at).
function assertLegacyInbound(item) {
  if (isInactive(item)) {
    throw _err('VALIDATION_ERROR', 'الصنف "' + itemLabel(item) + '" غير نشط — لا يمكن إدخال مخزون جديد له');
  }
  const mode = item && item.tracking_mode;
  if (mode === 'lot' || mode === 'expiry') {
    throw _err('WRITER_NOT_LOT_AWARE', 'الصنف "' + itemLabel(item) + '" يخضع لتتبع الدفعات — استخدم إذن استلام warehouse-v2 لإنشاء الدفعات (المسار القديم لا يحفظ هوية الدفعة/الصلاحية)');
  }
}

module.exports = { isInactive, itemLabel, assertInflowAllowed, assertOutflowAllowed, assertLegacyInbound };
