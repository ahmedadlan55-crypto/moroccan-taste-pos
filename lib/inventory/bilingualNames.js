...
const latinTokens = (s) => (String(s || '').match(/[A-Za-z]+/g) || []).map((t) => t.toLowerCase());

/**
 * Is this row suffering from the defect? Two distinct shapes qualify.
 *
 * (1) UNTOUCHED — `name` is Latin, and `name_en` is either empty or identical
 *     to it. The identical case is subtle but load-bearing: a previous run
 *     sets `name_en` first, and a row whose vocabulary the dictionary did not
 *     know keeps its English `name`. Demanding an EMPTY `name_en` would freeze
 *     those rows out of every future run, so a better dictionary could never
 *     reach them. `name_en === name` is exactly the fingerprint of "we moved
 *     it but could not translate it".
 *
 * (2) PARTIAL — the first production run left 115 rows with a MIXED name:
 *     «صدور دجاج Boneless», Arabic plus untranslated English residue. Those
 *     are invisible to isLatinOnly forever, yet `name_en` still holds the
 *     pristine English, so the row can simply be re-translated from it.
 *
 *     The guard on (2) is that every Latin word left in `name` must also
 *     appear in `name_en`. That proves `name` is still a pure derivative of
 *     our own output. If a human typed anything new — a brand, a note, a
 *     correction — a token appears that `name_en` does not have, and the row
 *     is left alone. Re-translating is only safe while nobody has edited it.
 *
 * A row with a DIFFERENT human-typed `name_en` is never a candidate under (1):
 * fixing it would mean choosing which of two human-entered values to destroy.
 */
function isCandidate(row) {
  ...
  if (dict.isLatinOnly(name)) return en === '' || en === name;

  // (2) our own partial output, still carrying English residue
  if (!en || !dict.isLatinOnly(en)) return false;
  const residue = latinTokens(name);
  if (!residue.length) return false;                 // fully Arabic — done, leave it
  const source = new Set(latinTokens(en));
  return residue.every((t) => source.has(t));
}

/**
 * The English to translate FROM. For a partial row that is `name_en` (the
 * pristine original), not `name` (our half-finished output) — re-translating
 * the output would compound the loss instead of repairing it.
 */
function sourceEnglish(row) {
  const name = trim(row.name);
  return dict.isLatinOnly(name) ? name : trim(row.name_en);
}
...
function planBilingualNames(rows, opts = {}) {
  ...
  const plan = work.map((r) => {
    const english = sourceEnglish(r);   // `name` when untouched, `name_en` when partial
    const t = dict.toArabic(english);
    const hadSku = !!trim(r.sku);
    return {
      id: r.id,
      oldName: r.name,
      newNameEn: english,                // verbatim — it IS the English
      newNameAr: t.ar || english,
      sku: trim(r.sku) || nextSku(),
      hadSku,
      repair: !dict.isLatinOnly(trim(r.name)),   // a re-translation, not a first pass
      matched: t.matched,
      tokens: t.tokens,
  ...
  }).filter((p) => p.newNameAr !== p.oldName || !p.hadSku);   // skip provable no-ops
  ...
  return {
    ...
    stats: {
      ...
      repairs: plan.filter((p) => p.repair).length,
