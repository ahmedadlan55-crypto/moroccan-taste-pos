...
async function calculateDailyAttendance(employeeId, date) {
  const [att] = await db.query(
    ...
    [employeeId, date]);
  const exceptionsForNoAtt = await getActiveExceptions(employeeId, date);
  if (!att.length) {
    // Check for excuse_absence exception
    const excused = exceptionsForNoAtt.find(function(e) { return e.exception_type === 'excuse_absence'; });
    if (excused) {
      return { status: 'excused', lateMinutes: 0, overtimeMinutes: 0, earlyLeaveMinutes: 0, workMinutes: 0, excused: true, excuseReason: excused.reason };
    }
    return { status: 'absent', lateMinutes: 0, overtimeMinutes: 0, earlyLeaveMinutes: 0, workMinutes: 0 };
  }
...
async function calculateMonthlyAttendance(employeeId, year, month) {
  ...
  const summary = {
    ...
    totalWorkMinutes: 0, presentDays: 0, absentDays: 0, excusedDays: 0, days: []
  };
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (result.status === 'excused') summary.excusedDays++;
  }
  return summary;
}
