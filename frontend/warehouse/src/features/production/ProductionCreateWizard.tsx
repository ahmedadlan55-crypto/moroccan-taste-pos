...
import { useBoms, usePreviewAvailability, useProductionDetail, useProductionMutations } from "@/lib/hooks/useProduction";
import { formatCurrency, formatQty, formatNumber } from "@/lib/formatters";
import type { BomOption } from "@/lib/adapters/production.adapter";
...
export function ProductionCreateWizard() {
  ...
  async function submit() {
    ...
    if (editId && existing.data) {
      ...
      navigate(`/production/${editId}`, { replace: true });
    } else {
      const r = await create.mutateAsync(input);
      navigate(`/production/${r.id}`, { replace: true });
    }
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button variant="ghost" onClick={() => navigate("/production")}>عودة للقائمة</Button>}
      />
