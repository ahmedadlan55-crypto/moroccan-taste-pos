...
import { useReportCatalog } from "@/modules/inventory/lib/hooks/useReport";
...
export function ReportsPage() {
  ...
  return (
    <div>
      <PageHeader
        eyebrow={t("inventoryRest.reports.eyebrow")}
        title={t("inventoryRest.reports.title")}
        subtitle={t("inventoryRest.reports.subtitle")}
      />
      ...
      {data && (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {data.map((r) => {
            const typeKey = `inventoryRest.reports.type.${r.type}`;
            const title = t(typeKey);
            return (
              <Link
                key={r.type}
                to={`/reports/${r.type}`}
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-sm font-extrabold text-slate-900">{title === typeKey ? r.label : title}</div>
                    {r.adminOnly && (
                      <div className="mt-1 inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-bold text-amber-700">
                        <ShieldAlert className="h-3 w-3" /> {t("inventoryRest.reports.adminOnly")}
                      </div>
                    )}
                  </div>
                </div>
