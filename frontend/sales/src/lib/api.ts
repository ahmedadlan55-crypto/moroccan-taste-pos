...
// Every MUTATION carries a fresh Idempotency-Key (safe retry) and, where a
// version is known, an If-Match header (optimistic concurrency → 409 on stale).
import { apiClient, type RequestOptions } from "./api-client";
import type {
  Customer, Customer360, Dashboard, DataResponse, Invoice, ListResponse,
...
export const o2cApi = {
  ...
  customerStatement: (id: string, params: Params, signal?: AbortSignal) => apiClient.get<DataResponse<{ customerId: string; from: string; to: string; opening: number; closing: number; lines: unknown[] }>>(`${B}/customers/${id}/statement`, { params, signal }),
