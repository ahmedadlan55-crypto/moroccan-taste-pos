...
const PERMISSIONS = {
  ...
  // ─── Reply (one per stage rule) ────────────────────────────────────────
  'txn.reply': {
    check: (user, ctx) => {
      const t = ctx.txn;
      if (!t) return false;
      // Creator can always reply on returned (to clarify)
      ...
      // Current assignee can reply (one per stage)
      if (_isAssignee(user, t)) {
        if (_actorAlreadyRepliedThisStage(user, ctx)) return false;
        return true;
      }
      // Recipients can reply
      if ((ctx.recipientUsernames || []).includes(user.username)) {
        if (_actorAlreadyRepliedThisStage(user, ctx)) return false;
        return true;
      }
      // Even creator can reply during in_progress (to add clarification)
