...
// Tier A.2 corrective gate — 'auditor' was in GRANT_ONLY_ROLES, meaning
// routes/auth.js's create/update paths (both gated by ROLES.isAssignable()
// below) rejected it outright. It already had real capability grants
// seeded (finance.reports.view / finance.gl.view / finance.bankrec.view —
// see db/migrations/finance/capabilities.js) that no account could ever
// actually hold: every one of those grants was unreachable through the
// product's own user-management UI/API. users.role's ENUM already includes
// 'auditor' (db/migrations/0020_auditor_role.sql) — this is the other half
// of making the role real, not just present in a column type.
const ASSIGNABLE_ROLES = Object.freeze([
  'admin', 'cashier', 'manager', 'custody', 'employee',
  'accountant', 'finance', 'sales', 'auditor',
]);

const GRANT_ONLY_ROLES = Object.freeze(['hr', 'inventory', 'purchasing']);
