 ...
 *
 * `discount_reason` USED to be the standing example here: reserved in the
 * contract with an empty `facts` map, `groupable: true`, and rejected by the
 * planner against every metric — a dimension nothing could group by. It is now
 * backed by a real projection (analytics_order_facts.discount_reason, written
 * from sales.discount_name) and supported on the `order` fact like any other
 * order attribute. The principle the example illustrated is unchanged: this
 * module reports fact support HONESTLY, whatever it happens to be, and the
 * caller decides how to present a dimension nothing can group by.
 *
