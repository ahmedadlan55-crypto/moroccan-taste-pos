 ...
 *     label,            how to resolve display labels (optional; label lookups
 *                       happen OUTSIDE the fact query). TWO shapes:
 *                       { table, idCol, cols, enCol? } — a master-table read.
 *                         cols[0] is the PRIMARY (Arabic) name. `enCol` — which
 *                         MUST be one of `cols` — names the English column; a
 *                         descriptor that declares none simply has no English
 *                         name to offer and stays on the primary in every
 *                         language.
 *                       { resolver: 'person', idCol } — NOT a table read; see
 *                         PERSON_LABEL below.
 *   }
 *
...
/**
 * The label descriptor every `kind: 'employee'` dimension shares.
 *
 * Employee fact columns (analytics_order_facts.created_by / salesperson /
 * void_by / …) are VARCHAR usernames — the login id, not a surrogate key — so
 * without a descriptor a report grouped by cashier printed "2004" where a human
 * expects a name. That is the exact defect lib/displayName.js was written to
 * kill on the receipt (see its header), and the reason this is NOT
 * `{ table: 'users', idCol: 'username', cols: ['full_name'] }`:
 *
 *   the repo's ONE rule for "what is this person called" spans TWO stores —
 *   users.full_name → settings.user_meta[username].name → username — and a
 *   plain users read cannot see the second one. An account whose name only ever
 *   reached the legacy JSON store would then be called one thing on its receipt
 *   and a different thing in the analytics report. Two display names for the
 *   same person in the same product is the failure this avoids.
 *
 * `resolver: 'person'` therefore routes the dimension to that helper
 * (services/analytics/QueryService.attachLabels), which is the only place the
 * rule is implemented.
 */
const PERSON_LABEL = Object.freeze({ resolver: 'person', idCol: 'username' });
...
const DIMENSIONS = Object.freeze([
  ...
  // ── scope ─────────────────────────────────────────────────────────────────
  {
    ...
    label: Object.freeze({
      table: 'branches', idCol: 'id', cols: Object.freeze(['name', 'name_en']), enCol: 'name_en',
    }),
  },
  {
  ...
  // ── order attributes ──────────────────────────────────────────────────────
  {
    ...
    label: Object.freeze({
      table: 'sales_channels', idCol: 'id', cols: Object.freeze(['name', 'name_en']), enCol: 'name_en',
    }),
  },
  {
  ...
  // ── catalog ───────────────────────────────────────────────────────────────
  {
    ...
    // `name` is the Arabic primary; `name_en` is DECLARED as the English column
    // rather than inferred from cols[1] — position is not a contract, and a
    // resolver that guessed would hand an English reader whatever the second
    // column happened to be. name_en is NULLABLE (menu.name_en_source /
    // name_en_needs_review exist precisely because it is backfilled), so the
    // resolver must also say when it fell back — see attachLabels.
    label: Object.freeze({
      table: 'menu', idCol: 'id', cols: Object.freeze(['name', 'name_en']), enCol: 'name_en',
    }),
  },
