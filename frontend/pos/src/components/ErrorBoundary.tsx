...
// and the active language is read directly from localStorage via a local,
// dependency-free try/catch instead of any hook or context.
interface Props {
  children: ReactNode;
}
interface State {
...
export class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    // itself is what crashed, so this deliberately does not use useLang().
    const lang = (() => {
      try {
        return localStorage.getItem("pos_lang") === "en" ? "en" : "ar";
      } catch {
        return "ar";
      }
    })();
