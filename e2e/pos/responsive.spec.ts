...
test("cashier header stays labelled, contained and touch-safe", async ({ page }) => {
  const env = readEnv();
  ...
  await page.addInitScript((value) => localStorage.setItem("pos_token", value), token);
  await page.goto("/pos/");
  ...
  await quick.getByText("المزيد", { exact: true }).click();
  await expect(page.getByRole("link", { name: "العودة للنظام الرئيسي" })).toBeVisible();
  const menuContained = await page.getByRole("link", { name: "العودة للنظام الرئيسي" }).evaluate((link) => {
