...
async function recomputeLevels(conn, opts = {}) {
  const companyId = opts.companyId == null ? null : String(opts.companyId);
  const [accounts] = await conn.query(
    'SELECT id, code, parent_id, level FROM gl_accounts' +
      (companyId ? " WHERE COALESCE(company_id,'CO-MAIN') = ?" : ''),
    companyId ? [companyId] : []
  );
  ...
  for (const a of scope) {
    ...
    await conn.query(
      'UPDATE gl_accounts SET level = ? WHERE id = ?' +
        (companyId ? " AND COALESCE(company_id,'CO-MAIN') = ?" : ''),
      companyId ? [d, a.id, companyId] : [d, a.id]
    );
    updated++;
  }
