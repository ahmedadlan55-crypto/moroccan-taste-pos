...
async function postJournal(db, spec) {
  try {
    ...
    // v6.0.1 Wave A.3 — FOR UPDATE locks the last row of the day's series
    // so two concurrent posters can't both read serial=N and write N+1.
    // The UNIQUE key on journal_number (added in v6.0.1 migration) is the
    // final safety net; on the very rare race we retry with serial+1.
    const ymd = jdate.replace(/-/g, '');
    let jNum, jId, inserted = false, retries = 0;
    while (!inserted && retries < 4) {
      let lastJ;
      try {
        [lastJ] = await db.query(
          `SELECT journal_number FROM gl_journals WHERE journal_number LIKE ? ORDER BY created_at DESC LIMIT 1 FOR UPDATE`,
          ['JV-' + ymd + '-%']);
      } catch (lockErr) {
        // Connection might not be inside a transaction (older callers) — fall back to plain SELECT
        [lastJ] = await db.query(
          `SELECT journal_number FROM gl_journals WHERE journal_number LIKE ? ORDER BY created_at DESC LIMIT 1`,
          ['JV-' + ymd + '-%']);
      }
      let serial = 1 + retries;
      if (lastJ.length) {
        const m = lastJ[0].journal_number.match(/-(\d+)$/);
        if (m) serial = parseInt(m[1]) + 1 + retries;
      }
      jNum = 'JV-' + ymd + '-' + String(serial).padStart(4, '0');
      jId = genId('JRN');
      // The actual INSERT happens further below; we just compute the number
      // here. If the INSERT fails with duplicate-key, the outer caller (or
      // the catch below) will retry. To handle that, we tentatively break
      // and let the INSERT attempt take the lock.
      inserted = true; // we leave the retry loop after computing jNum
      break;
    }
    if (!jNum) {
      jNum = 'JV-' + ymd + '-' + String(Date.now()).slice(-4);
      jId = genId('JRN');
    }
