...
const { nextFlatJENumber } = require('./glPosting'); // FC-B1 atomic JE numbering
...
// ──────────────────────────────────────────────────
// v5.11.14 — Realigned to v5.11.8 IFRS template:
//   611 = Salaries & Wages (under 61 Branch Operating)
//   612 = Employee Benefits (GOSI / health insurance)
//   212 = Accrued Expenses (salaries payable lives here, NOT under 22)
//   216 = GOSI Payable (template has dedicated bucket)
//   115 = Custody & Advances (employee advances live here)
//   42  = Other Income.
const SALARY_ACCOUNTS = [
  // Expense accounts (5xxx) — administrative payroll posts to 611/612.
  { code: '521100', nameAr: 'مصروف الرواتب والأجور', type: 'expense', parent: '521000' },
  { code: '521200', nameAr: 'مصروف البدلات',          type: 'expense', parent: '521000' },
  { code: '521300', nameAr: 'مصروف الإضافي',          type: 'expense', parent: '521000' },
  { code: '521400', nameAr: 'مصروف التأمينات - حصة الشركة', type: 'expense', parent: '521000' },
  // Liability accounts — accrued payroll is a CURRENT liability, not 22.
  { code: '212100', nameAr: 'رواتب وأجور مستحقة', type: 'liability', parent: '212000' },
  { code: '213300', nameAr: 'التأمينات الاجتماعية - حصة الموظف', type: 'liability', parent: '213000' },
  // Asset accounts — employee advances belong under Custody & Advances.
  { code: '112300', nameAr: 'سلف الموظفين',         type: 'asset', parent: '112000' },
  // Revenue accounts
  { code: '419200', nameAr: 'إيرادات جزاءات ومخالفات', type: 'revenue', parent: '419000' },
];
...
async function ensurePayrollAccounts() {
  // Canonical parents are seeded/migrated by the governed chart. These calls
  // are a fresh-database safety net and never create short legacy roots.
  await ensureParentAccount('521000', 'الرواتب والمزايا', 'expense');
  await ensureParentAccount('212000', 'المصروفات والمبالغ المستحقة', 'liability');
  await ensureParentAccount('213000', 'الضرائب والاستحقاقات الحكومية', 'liability');
  await ensureParentAccount('112000', 'الذمم المدينة والسلف', 'asset');
  await ensureParentAccount('419000', 'إيرادات أخرى', 'revenue');
...
async function createAndPostJournal(opts) {
  ...
  // FC-B1 — atomic JE numbering (was ORDER BY created_at DESC, which raced on
  // the 1s created_at and could even parse a JV-* number).
  const journalNumber = await nextFlatJENumber();
...
async function postPayrollJournals(runId, username) {
  ...
  // en-GB, not ar-SA: this string is written into the GL entry DESCRIPTION, and
  // 'ar-SA' renders Arabic-Indic digits ("يوليو ٢٠٢٦"), so the posting period of
  // a payroll journal read differently from every other date in the ledger.
  const periodStr = new Date(run.year, run.month-1, 1).toLocaleDateString('en-GB', { month:'long', year:'numeric', calendar:'gregory', numberingSystem:'latn' });
  ...
  // ═══ قيد 1: الاستحقاق ═══
  const accrualLines = [];
  if (totalBasic > 0) accrualLines.push({ accountId: accounts['521100'], accountCode:'521100', accountName:'مصروف الرواتب والأجور', debit: totalBasic, credit: 0 });
  if (totalAllowances > 0) accrualLines.push({ accountId: accounts['521200'], accountCode:'521200', accountName:'مصروف البدلات', debit: totalAllowances, credit: 0 });
  if (totalOvertime > 0) accrualLines.push({ accountId: accounts['521300'], accountCode:'521300', accountName:'مصروف الإضافي', debit: totalOvertime, credit: 0 });
  accrualLines.push({ accountId: accounts['212100'], accountCode:'212100', accountName:'رواتب وأجور مستحقة', debit: 0, credit: totalGross });
  ...
  if (totalDeductionsForJournal > 0) {
    dedLines.push({ accountId: accounts['212100'], accountCode:'212100', accountName:'رواتب وأجور مستحقة', debit: totalDeductionsForJournal, credit: 0 });
    if (totalAdvance > 0) dedLines.push({ accountId: accounts['112300'], accountCode:'112300', accountName:'سلف الموظفين', debit: 0, credit: totalAdvance });
    if (totalInsurance > 0) dedLines.push({ accountId: accounts['213300'], accountCode:'213300', accountName:'التأمينات الاجتماعية - حصة الموظف', debit: 0, credit: totalInsurance });
    if (totalPenalties > 0) dedLines.push({ accountId: accounts['419200'], accountCode:'419200', accountName:'إيرادات جزاءات ومخالفات', debit: 0, credit: totalPenalties });
    if (totalFixedDed > 0) dedLines.push({ accountId: accounts['419200'], accountCode:'419200', accountName:'إيرادات جزاءات ومخالفات', debit: 0, credit: totalFixedDed });
...
async function postPayrollPaymentJournal(runId, bankAccountId, username) {
  ...
  // en-GB — see postPayrollJournals: this lands in the GL entry description.
  const periodStr = new Date(run.year, run.month-1, 1).toLocaleDateString('en-GB', { month:'long', year:'numeric', calendar:'gregory', numberingSystem:'latn' });
  ...
  const lines = [
    { accountId: accounts['212100'], accountCode:'212100', accountName:'رواتب وأجور مستحقة', debit: totalNet, credit: 0 },
