...
export const header = {
  ...
  orphanQueue: {
    label: "كاشير آخر",
    title: (p: { who: string }) => `عمليات غير مُزامَنة تخص ${p.who} — تُرسَل عند دخوله، أو يفرج عنها مدير`,
    titleSales: (p: { count: number; who: string }) =>
      `${p.count} عملية بيع مكتملة باسم ${p.who} لم تصل الخادم بعد — تُرسَل عند دخوله أو بدخول مدير. لم يضع منها شيء.`,
  },
