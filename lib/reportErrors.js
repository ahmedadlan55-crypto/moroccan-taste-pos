...
// Deliberate, coded errors. A handler throws one of these to say something TRUE
// and specific; anything else is an unexplained fault and must not be dressed up
// as an answer.
//
// 503 is here alongside the 4xx because "this capability is not installed on
// this host" (no PDF renderer; an unmapped governed ledger role) is a precise,
// actionable statement — and one the client must be able to branch on to offer
// a fallback. Collapsing it into the generic 500 would tell the user "something
// went wrong" when the truth is "this button cannot work here, use the other
// one".
const KNOWN_HTTP = new Set([400, 403, 404, 409, 413, 422, 429, 503]);
