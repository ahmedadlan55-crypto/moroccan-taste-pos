...
const TRUSTED_EXPRESSIONS = new Set([
  ...
  // A sales line carries its own warehouse; the document's is the fallback. A
  // line with NEITHER resolves to NULL, which fails every `= ?` and every
  // `IN (...)` — so under a restricted scope it is excluded rather than
  // leaking into a warehouse the reader may not see. Fail closed.
  'COALESCE(d.warehouse_id, doc.warehouse_id)',
]);
