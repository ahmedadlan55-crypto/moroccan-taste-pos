...
const METRICS = Object.freeze([
  ...
  // Cost actually reversed out of COGS. A return can be accepted without
  // putting the goods back on the shelf (food waste, damaged goods, etc.). In
  // that case SalesReturnService deliberately posts NO COGS reversal, so using
  // the all-lines returns_cogs snapshot would overstate profit. Keep the old
  // metric for audit/history and use this one for accounting profitability.
  {
    id: 'returns_cogs_reversed', kind: 'additive', fact: 'return',
    sql: 'SUM(rl.cogs_reversed_amount)',
  ...
  // A restocked return changes COGS, so its snapshot must be trustworthy too.
  // Non-restocked lines intentionally do not enter this gap: their sale COGS
  // remains on the books and there is no return-cost reversal to estimate.
  {
    id: 'uncosted_returns_net', kind: 'additive', fact: 'return',
    sql: 'SUM(CASE WHEN COALESCE(rl.restock, 0) = 1 AND (rl.cogs_reversed_amount IS NULL OR COALESCE(rl.cost_snapshot, 0) = 0) THEN rl.net_amount ELSE 0 END)',
    format: 'money', equationKey: 'sum', version: 1, requiresCap: CAP_COST,
  },
  ...
  // The figure a VAT return actually carries. `vat_amount` is VAT on SALES
  // only; a period's liability is that less the VAT credited back on returns.
  // Reporting `vat_amount` as "the VAT" overstates the liability by the whole
  // of `returns_vat`.
  {
    id: 'net_vat', kind: 'derived', equationKey: 'netVat',
    inputs: Object.freeze(['vat_amount', 'returns_vat']),
    format: 'money', version: 1,
  },
  ...
  // Explicit AFTER-RETURN profitability. The legacy gross_profit/margin_pct
  // remain the before-return pair for backward compatibility. cogs_after_returns
  // is a first-class statement line so a KPI row can reconcile visibly; the
  // profit metrics repeat its ADDITIVE operands because planner inputs may not
  // be derived metrics. All three read the exact amount frozen by the posting
  // transaction — exactly the cost that entered the COGS reversal journal.
  {
    id: 'cogs_after_returns', kind: 'derived', equationKey: 'cogsAfterReturns',
    inputs: Object.freeze(['cogs', 'returns_cogs_reversed']),
    format: 'money', version: 1, requiresCap: CAP_COST,
  },
  {
    id: 'gross_profit_after_returns', kind: 'derived',
    equationKey: 'grossProfitAfterReturns',
    inputs: Object.freeze(['net_ex_vat', 'cogs', 'returns_net', 'returns_cogs_reversed']),
    format: 'money', version: 1, requiresCap: CAP_COST,
  },
  {
    id: 'margin_pct_after_returns', kind: 'derived',
    equationKey: 'marginPctAfterReturns',
    inputs: Object.freeze(['net_ex_vat', 'cogs', 'returns_net', 'returns_cogs_reversed']),
    format: 'percent', version: 1, requiresCap: CAP_COST,
  },
  {
