...
const PERMISSIONS = {
  ...
  'txn.delete': {
    check: (user, ctx) => {
      const t = ctx.txn;
      if (!t) return false;
      if (t.created_by !== user.username) return false;
      // Allowed in draft, created, or returned (not after others have acted on it productively)
      return [STATES.DRAFT, STATES.CREATED, STATES.RETURNED].includes(t.status);
    }
  },

  // ─── Force-delete (developer only) ─────────────────────────────────────
  'txn.delete.force': {
    check: (user) => !!user.isDeveloper
  },
  ...
  // ─── Reply (V4.5: anyone who can VIEW can reply, subject to one-per-stage) ─
  // Closed/rejected: still allow reply (post-mortem comments are useful)
  'txn.reply': {
    check: (user, ctx) => {
      const t = ctx.txn;
      if (!t) return false;
      // Admin/developer can always reply (override one-per-stage)
      if (user.role === 'admin' || user.isDeveloper) return true;
      // Anyone who can VIEW the transaction can reply, BUT subject to one-per-stage
      const canView =
      ...
      // Creator can always reply on returned (to clarify) — bypasses one-per-stage for them
      if (t.status === STATES.RETURNED && t.created_by === user.username) return true;
      // Everyone else: one reply per stage
      if (_actorAlreadyRepliedThisStage(user, ctx)) return false;
      return true;
    }
  },
...
function can(user, permission, ctx = {}) {
  ...
  // Always allow developer everything (can be revoked per-permission if needed)
  if (user && user.isDeveloper && permission !== 'txn.delete.force') {
    // Even developer needs the txn to exist for view/edit checks
